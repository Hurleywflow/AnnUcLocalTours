//! Tours
export type TourType = {
	id: string;
	title?: string;
	subTitle?: Array<{
		id: string;
		title?: string;
		price?: string;
		departs?: string;
		schedule?: string[];
	}>;
	description?: Array<string>;
	imageUrl?: string[];
	link?: string;
	price?: string;
	rating?: number;
};
export const ToursAustralia: TourType[] = [
	{
		id: "au1",
		title: "Sydney City Discover ",
		price: "$160/adult. Departure from 10 guests.",
		// subTitle: [
		// 	{
		// 		id: "au1",
		// 		title: "DANDENONG AUTUMN – YARRA VALLEY",
		// 		price: "$150/adult, children < 10 years old $120",
		// 		schedule: [
		// 			"Sydney, Australia, is an amazing city celebrated for its iconic landmarks, breathtaking harbour, lively culture, and diverse attractions. Here’s why Sydney should top your list for an unforgettable adventure:",
		// 			"Sydney, Australia is a city that offers a variety of cultural experiences, from the iconic Sydney Opera House to the vibrant nightlife scene of the city’s famous CBD pub scene. Sydney is also a city that offers a variety of cultural experiences, from the iconic Sydney Opera House to the vibrant nightlife scene of the city’s famous CBD pub scene.",
		// 		],
		// 	},
		// ],
		description: [
			"Sydney, Australia, is an amazing city celebrated for its iconic landmarks, breathtaking harbour, lively culture, and diverse attractions. Here’s why Sydney should top your list for an unforgettable adventure:",
			"Sydney boasts iconic landmarks like the Sydney Opera House and the Sydney Harbour Bridge. The coastline offers stunning beaches such as Bondi and Manly, as well as picturesque coastal walks. The city has a thriving cultural scene with art galleries, museums, and live performances. You’ll find lush parks and gardens like the Royal Botanic Garden and Centennial Park. Exciting attractions include the Taronga Zoo and various outdoor activities. The food scene is diverse, featuring fine dining, seafood markets, and multicultural cuisine. Sydney is also host to a variety of festivals and events throughout the year, and it's known for its welcoming locals and cosmopolitan atmosphere.",
			"Sydney is a destination that offers the perfect blend of iconic landmarks, natural beauty, vibrant culture, and exciting activities. Whether you’re eager to explore world-famous sights, unwind on beautiful beaches, immerse yourself in the arts, or indulge in delicious food, Sydney has something for every adventurer. So, prepare to experience the magic of Sydney and find out why it’s one of the world’s most captivating cities!",
		],
		imageUrl: [
			"/Images/tours/sydney/1.jpg",
			"/Images/tours/sydney/2.jpg",
			"/Images/tours/sydney/3.jpg",
			"/Images/tours/sydney/4.jpg",
			"/Images/tours/sydney/5.jpg",
			"/Images/tours/sydney/6.jpg",
			"/Images/tours/sydney/7.jpg",
			"/Images/tours/sydney/8.jpg",
			"/Images/tours/sydney/9.jpg",
			"/Images/tours/sydney/10.jpg",
			"/Images/tours/sydney/11.jpg",
			"/Images/tours/sydney/12.jpg",
			"/Images/tours/sydney/13.jpg",
			"/Images/tours/sydney/14.jpg",
			"/Images/tours/sydney/15.jpg",
			"/Images/tours/sydney/16.jpg",
			"/Images/tours/sydney/17.jpg",
			"/Images/tours/sydney/18.jpg",
		],
		link: "https://www.netcodedev.com",
		rating: 4,
	},
	{
		id: "au2",
		title: "Melbourne",
		price: "",
		subTitle: [
			{
				id: "au2.1",
				title: "DANDENONG AUTUMN – YARRA VALLEY",
				price: "$150/adult, children < 10 years old $120",
				departs: "Every Monday and Thursday",
				schedule: [
					"🚗 Ancient steam train – Botanic Garden – Ancient town - Waterfall – Chocolate & ice cream factory.",
				],
			},
			{
				id: "au2.2",
				title: "BALLARAT AUTUMN",
				price: "$150/adult, children < 10 years old $120",
				departs: "Every Tuesday",
				schedule: [
					"🚗 Fruit picking – gold mine – Ballarat town – Art museum – botanical garden.",
				],
			},
			{
				id: "au2.3",
				title: "GREAT OCEAN ROAD",
				price: "$140/adult, children < 10 years old $110",
				departs: "Every week",
				schedule: [
					"🚗 Rose garden, ancient castle - 12 apostles - Loch Goerge narrow gap – Appolo bay – Lorn –– Anglesea beach.",
					" *** (If the tour does not go to the rose garden and ancient castle, the price is $120/adult)",
					" *** Guests who want to take a helicopter ride can buy their own tickets at the counter for $165/adult.",
				],
			},
			{
				id: "au2.4",
				title: "PENINSULAR",
				price: "$130/adult, children < 10 years old $100",
				departs: "Monday and Saturday every week",
				schedule: [
					"🚗 Cape Schanck Light house – Gape farm Moonlit zoo – Mornington beach.",
				],
			},
			{
				id: "au2.5",
				title: "BUFFLING BILLY RED HILL - PENINSULAR",
				price: "$160/adult, children < 10 years old $120",
				departs: "Monday and Friday every week",
				schedule: [
					"🚗 Puffing Billy – Cape Schanck Lighthouse (Peninsula) – Gape farm – Botanic Gardent Dandenong.",
				],
			},
			{
				id: "au2.6",
				title: "WILSON PROMONTORY",
				price: "$130/adult, children < 10 years old $100",
				departs: "Sunday every week",
				schedule: [
					"🚗 Wilson Prom – Tidal river – Norman Bay – Squeaky – Dunes of the Drift.",
					" *** (Customers who want to take the boat to the island must buy their own tickets for $145/adult).",
				],
			},
			{
				id: "au2.7",
				title: "MACEDON FALL FESTIVAL",
				price: "$150/adult, children < 10 years old $130",
				departs: "Sunday every week",
				schedule: [
					"🚗 Macedon – Botanic Garden – Hanging Rock",
					" *** (Tour price includes tickets to Botanic Garden and Hanging Rock).",
				],
			},
			{
				id: "au2.8",
				title: "BRIGHT AUTUMN",
				price: "$150/adult, children < 10 years old $130",
				departs: "Every week",
				schedule: ["🚗 Bright – Ovens river – Lest we forget."],
			},
		],
		description: [
			"Melbourne, Australia, is an exciting and vibrant city that perfectly combines culture, natural beauty, and thrilling attractions. Whether you're passionate about art, food, nature, or adventure, Melbourne has something for you. Here's why Melbourne should be your next adventure destination:",
			"Melbourne is renowned for its diverse culture and arts scene, boasting art galleries, theatres, and music venues. The city is a culinary paradise with a top-notch food and coffee scene, offering a wide range of dining options and a famous coffee culture. What's more, Melbourne boasts stunning parks and gardens, thrilling attractions and activities, coastal charm, and picturesque drives. The city hosts lively festivals and events throughout the year and has a welcoming, cosmopolitan vibe with friendly locals and efficient public transport.",
			"Melbourne is a destination that blends cultural richness, culinary pleasures, natural beauty, and thrilling activities. Whether you want to immerse yourself in the arts, savour world-class dining, explore scenic landscapes, or enjoy vibrant festivals, Melbourne has something for every adventurer. So pack your bags and get ready to uncover why Melbourne stands out as one of Australia's most captivating and dynamic cities!",
		],
		imageUrl: [
			"/Images/tours/melbourne/1.jpg",
			"/Images/tours/melbourne/2.jpg",
			"/Images/tours/melbourne/3.jpg",
			"/Images/tours/melbourne/4.jpg",
			"/Images/tours/melbourne/5.jpg",
			"/Images/tours/melbourne/6.jpg",
			"/Images/tours/melbourne/7.jpg",
			"/Images/tours/melbourne/8.jpg",
			"/Images/tours/melbourne/9.jpg",
			"/Images/tours/melbourne/10.jpg",
			"/Images/tours/melbourne/11.jpg",
			"/Images/tours/melbourne/12.jpg",
			"/Images/tours/melbourne/13.jpg",
			"/Images/tours/melbourne/14.jpg",
			"/Images/tours/melbourne/15.jpg",
			"/Images/tours/melbourne/16.jpg",
			"/Images/tours/melbourne/17.jpg",
			"/Images/tours/melbourne/18.jpg",
			"/Images/tours/melbourne/19.jpg",
		],
		link: "https://www.netcodedev.com",
		rating: 4,
	},
	{
		id: "au3",
		title: "Adelaide",
		description: [
			"Adelaide, the delightful capital of South Australia, often goes unnoticed despite being a treasure trove of natural beauty, rich culture, and unforgettable experiences. Here's why considering Adelaide for your next adventure is a great idea:",
			"Adelaide boasts breathtaking views and natural beauty, including pristine beaches, lush landscapes, and serene gardens. The city is steeped in culture and heritage, offering art galleries, museums, and historical sites to explore. It's also a hub for top-notch wine and gourmet experiences, featuring nearby wine regions and a diverse dining scene. Visitors can partake in thrilling attractions and activities, such as wildlife encounters, outdoor adventures, and lively festivals throughout the year. Known for its amiable locals and laid-back lifestyle, Adelaide is a welcoming and effortlessly enjoyable city to explore.",
			"Adelaide is a destination that seamlessly combines natural beauty, rich culture, world-class wine, and exciting attractions. Whether you're seeking relaxation on the beach, exploration of lush vineyards, or immersion in vibrant festivals, Adelaide offers something for every adventurer. So, pack your bags and be prepared to uncover why Adelaide stands out as one of Australia's most charming and diverse destinations!",
		],
		imageUrl: [
			"/Images/tours/adelaide/1.jpg",
			"/Images/tours/adelaide/2.jpg",
			"/Images/tours/adelaide/3.jpg",
			"/Images/tours/adelaide/4.jpg",
			"/Images/tours/adelaide/5.jpg",
			"/Images/tours/adelaide/6.jpg",
			"/Images/tours/adelaide/7.jpg",
			"/Images/tours/adelaide/8.jpg",
			"/Images/tours/adelaide/9.jpg",
			"/Images/tours/adelaide/10.jpg",
			"/Images/tours/adelaide/11.jpg",
			"/Images/tours/adelaide/12.jpg",
			"/Images/tours/adelaide/13.jpg",
			"/Images/tours/adelaide/14.jpg",
			"/Images/tours/adelaide/15.jpg",
			"/Images/tours/adelaide/16.jpg",
			"/Images/tours/adelaide/17.jpg",
		],
		link: "https://www.netcodedev.com",
		price: "",
		rating: 4,
	},
	// {
	// 	id: "au4",
	// 	title: "Brisbane",
	// 	description: [
	// 		"Brisbane, the capital of Queensland, Australia, is a vibrant and dynamic city that offers a perfect blend of natural beauty, cultural richness, and exciting attractions. Here’s why you should consider Brisbane for your next adventure:",
	// 		"Brisbane offers stunning river views, lush green spaces, and the iconic Mount Coot-tha for panoramic city vistas. The city boasts a thriving cultural scene with museums, galleries, and the Queensland Cultural Centre. Visitors can enjoy thrilling activities such as the Story Bridge Adventure Climb and wildlife encounters at the Lone Pine Koala Sanctuary. The diverse food scene includes markets, night markets, and craft breweries. Brisbane hosts various festivals and events throughout the year, and its friendly locals and laid-back lifestyle make it a welcoming destination.",
	// 		"Brisbane is a destination that offers a perfect mix of natural beauty, rich culture, thrilling attractions, and a friendly atmosphere. Whether you’re seeking adventure, relaxation, or cultural experiences, Brisbane has something for everyone. So, get ready to explore this vibrant city and discover why Brisbane is one of Australia’s most exciting destinations!",
	// 	],
	// 	imageUrl: [
	// 		"/Images/contact/form.webp",
	// 		"/Images/Demo/2.jpeg",
	// 		"/Images/Demo/3.jpeg",
	// 		"/Images/Demo/4.jpeg",
	// 		"/Images/Demo/5.jpeg",
	// 		"/Images/Demo/6.jpeg",
	// 	],
	// 	link: "https://www.netcodedev.com",
	// 	price: "Adult: $120, Children < 10 years old:  $90",
	// 	rating: 4,
	// },
	{
		id: "au5",
		title: "Gold Coast - Brisbane",
		description: [
			"The Gold Coast in Queensland, Australia, is an adventurer's paradise, offering an exhilarating mix of stunning natural beauty, vibrant culture, and thrilling attractions. Here’s why you should make the Gold Coast your next adventure destination:",
			"The Gold Coast offers spectacular beaches, thrilling theme parks, and diverse wildlife experiences. Visitors can explore the lush rainforests of the Hinterland, enjoy water sports, and indulge in vibrant nightlife. The region also boasts a rich cultural scene, delicious food and wine, and hosts various festivals and events throughout the year. The friendly locals and relaxed atmosphere make the Gold Coast a welcoming destination for travellers.",
			"The Gold Coast is a destination that offers a perfect blend of natural beauty, thrilling attractions, rich culture, and vibrant nightlife. Whether you're looking to relax on a beach, seek out adrenaline-pumping activities, or immerse yourself in local culture, the Gold Coast has something for every adventurer. So, get ready for an unforgettable adventure and discover why the Gold Coast is one of Australia's premier destinations!",
		],
		imageUrl: [
			"/Images/tours/gold-coast/1.jpg",
			"/Images/tours/gold-coast/2.jpg",
			"/Images/tours/gold-coast/3.jpg",
			"/Images/tours/gold-coast/4.jpg",
			"/Images/tours/gold-coast/5.jpg",
			"/Images/tours/gold-coast/6.jpg",
			"/Images/tours/gold-coast/7.jpg",
			"/Images/tours/gold-coast/8.jpg",
			"/Images/tours/gold-coast/9.jpg",
			"/Images/tours/gold-coast/10.jpg",
			"/Images/tours/gold-coast/11.jpg",
			"/Images/tours/gold-coast/12.jpg",
			"/Images/tours/gold-coast/13.jpg",
			"/Images/tours/gold-coast/14.jpg",
			"/Images/tours/gold-coast/15.jpg",
			"/Images/tours/gold-coast/16.jpg",
			"/Images/tours/gold-coast/17.jpg",
			"/Images/tours/gold-coast/18.jpg",
			"/Images/tours/gold-coast/19.jpg",
			"/Images/tours/gold-coast/20.jpg",
			"/Images/tours/gold-coast/21.jpg",
			"/Images/tours/gold-coast/22.jpg",
			"/Images/tours/gold-coast/23.jpg",
			"/Images/tours/gold-coast/24.jpg",
			"/Images/tours/gold-coast/25.jpg",
			"/Images/tours/gold-coast/26.jpg",
			"/Images/tours/gold-coast/27.jpg",
			"/Images/tours/gold-coast/28.jpg",
			"/Images/tours/gold-coast/29.jpg",
			"/Images/tours/gold-coast/30.jpg",
			"/Images/tours/gold-coast/31.jpg",
			"/Images/tours/gold-coast/32.jpg",
		],
		link: "https://www.netcodedev.com",
		price: "",
		rating: 4,
	},
	// {
	// 	id: "au6",
	// 	title: "Great Barrier Reef",
	// 	description: [
	// 		"The Great Barrier Reef is the largest coral reef system in the world, offering mesmerizing natural beauty and vibrant marine life. It spans over 2,300 kilometers and comprises more than 2,900 individual reefs and 900 islands, providing excellent visibility for appreciating the coral formations and marine ecosystems. It is home to thousands of species, including over 1,500 species of fish, 400 types of coral, and majestic creatures such as manta rays, sea turtles, and dugongs. You can explore the reef through snorkeling, scuba diving, or glass-bottom boat tours, and discover its many islands, each with its own unique charm. The reef also offers opportunities for eco-friendly tours, luxury stays on islands, and engaging with the traditional owners of the reef. Additionally, it provides a plethora of water sports and relaxation options, and is easily accessible from major gateway cities like Cairns, Townsville, and Port Douglas.",
	// 		"The Great Barrier Reef is a destination that captivates with its extraordinary natural beauty, incredible marine life, and diverse range of activities. Whether you’re an avid adventurer, a marine enthusiast, or simply looking to relax in paradise, the Great Barrier Reef offers an unparalleled experience. Embark on a journey to this magnificent wonder and create memories that will last a lifetime!",
	// 	],
	// 	imageUrl: [
	// 		"/Images/contact/form.webp",
	// 		"/Images/Demo/2.jpeg",
	// 		"/Images/Demo/3.jpeg",
	// 		"/Images/Demo/4.jpeg",
	// 		"/Images/Demo/5.jpeg",
	// 		"/Images/Demo/6.jpeg",
	// 	],
	// 	link: "https://www.netcodedev.com",
	// 	price: "Adult: $120, Children < 10 years old:  $90",
	// 	rating: 4,
	// },
	{
		id: "au7",
		title: "Perth",
		description: [
			"Perth, Australia, is an exceptional destination for any adventurer seeking a blend of natural beauty, vibrant culture, and unique attractions. Here’s why you should pack your bags and head to this stunning city:",
			"Perth, Australia offers breathtaking views and natural beauty, with opportunities for water-based activities and stunning sunsets at beaches like Cottesloe and Scarborough. The city also boasts a rich cultural tapestry, blending Indigenous heritage with modern Australian life through vibrant arts scenes and cultural institutions. Adventurers can enjoy wildlife at Rottnest Island, wine-tasting tours in Swan Valley, and adrenaline-pumping activities such as skydiving and surfing. Perth hosts a variety of festivals and events throughout the year, including the Perth Festival and Taste Great Southern Festival. The city’s friendly, laid-back atmosphere, warm hospitality, and safe environment make it an ideal place to visit.",
			"Perth is a destination that offers a perfect blend of natural beauty, rich culture, thrilling attractions, and a welcoming atmosphere. Whether you’re looking to relax on a beach, explore diverse landscapes, or immerse yourself in local culture, Perth has something for every adventurer. So, why wait? Start planning your unforgettable adventure to Perth today!",
		],
		imageUrl: [
			"/Images/tours/perth/1.jpg",
			"/Images/tours/perth/2.jpg",
			"/Images/tours/perth/3.jpg",
			"/Images/tours/perth/4.jpg",
			"/Images/tours/perth/5.jpg",
			"/Images/tours/perth/6.jpg",
			"/Images/tours/perth/7.jpg",
			"/Images/tours/perth/8.jpg",
			"/Images/tours/perth/9.jpg",
		],
		link: "https://www.netcodedev.com",
		price: "",
		rating: 4,
	},
	{
		id: "au8",
		title: "Tasmania",
		description: [
			"Tasmania, the island state of Australia, is a hidden gem that offers an unparalleled adventure with its pristine wilderness, rich history, and vibrant culture. Here’s why Tasmania should be your next travel destination:",
			"Tasmania offers stunning natural beauty, including World Heritage Wilderness areas and iconic national parks like Cradle Mountain-Lake St Clair and Freycinet. The island is home to unique wildlife such as the Tasmanian devil and boasts a rich history and culture, with a blend of convict history and Indigenous heritage. Culinary delights abound, with farm-to-table dining, renowned wineries, and artisanal products. Outdoor enthusiasts can enjoy hiking, kayaking, and wildlife watching, while the island hosts unique festivals and events like MONA FOMA and Taste of Tasmania. Tasmanians are known for their warm hospitality and friendliness, and local markets are a great way to experience the best of Tasmanian produce and crafts.",
			"Tasmania offers a unique blend of stunning natural beauty, rich history, vibrant culture, and delicious cuisine. Whether you’re an outdoor enthusiast, a history buff, a foodie, or simply seeking a serene escape, Tasmania has something to offer. Pack your bags and get ready to explore the wonders of this incredible island!",
		],
		imageUrl: [
			"/Images/tours/tasmania/1.jpg",
			"/Images/tours/tasmania/2.jpg",
			"/Images/tours/tasmania/3.jpg",
			"/Images/tours/tasmania/4.jpg",
			"/Images/tours/tasmania/5.jpg",
			"/Images/tours/tasmania/6.jpg",
			"/Images/tours/tasmania/7.jpg",
			"/Images/tours/tasmania/8.jpg",
			"/Images/tours/tasmania/9.jpg",
			"/Images/tours/tasmania/10.jpg",
			"/Images/tours/tasmania/11.jpg",
			"/Images/tours/tasmania/12.jpg",
			"/Images/tours/tasmania/13.jpg",
		],
		link: "https://www.netcodedev.com",
		price: "",
		rating: 4,
	},
	{
		id: "au9",
		title: "Uluru",
		description: [
			"Uluru, also known as Ayers Rock, is one of Australia's most iconic and awe-inspiring natural landmarks. Located in the heart of the Northern Territory, this massive sandstone monolith holds deep cultural significance for the Anangu people, the traditional custodians of the land. Visiting Uluru offers a unique blend of natural beauty, rich Aboriginal culture, and unforgettable experiences. Here’s why you should consider making Uluru your next adventure destination:",
			"Uluru is a unique geological feature with a height of 348m and dramatic color shifts. Uluru holds spiritual significance for the Anangu people, and cultural tours provide insights into Dreamtime stories and traditions. Visitors can experience magical sunrises and sunsets, witness the glowing colors of the Field of Light after dark, and explore the area on camel tours. The desert around Uluru is home to unique flora and fauna, with opportunities for wildlife spotting and birdwatching. Accommodations range from luxury resorts to budget-friendly options, all offering fine dining and cultural experiences.",
			"Uluru offers a unique and unforgettable travel experience, combining breathtaking natural beauty, rich Aboriginal culture, and a range of activities and adventures. Whether you’re watching the sunrise over the iconic rock, exploring ancient rock art, or learning about the traditions of the Anangu people, Uluru promises to be a journey of discovery and wonder. So, pack your bags and get ready to explore one of Australia’s most treasured and sacred destinations, Uluru.",
		],
		imageUrl: [
			"/Images/tours/uluru/1.jpg",
			"/Images/tours/uluru/2.jpg",
			"/Images/tours/uluru/3.jpg",
			"/Images/tours/uluru/4.jpg",
			"/Images/tours/uluru/5.jpg",
			"/Images/tours/uluru/6.jpg",
			"/Images/tours/uluru/7.jpg",
			"/Images/tours/uluru/8.jpg",
			"/Images/tours/uluru/9.jpg",
			"/Images/tours/uluru/10.jpg",
			"/Images/tours/uluru/11.jpg",
			"/Images/tours/uluru/12.jpg",
			"/Images/tours/uluru/13.jpg",
			"/Images/tours/uluru/14.jpg",
			"/Images/tours/uluru/15.jpg",
			"/Images/tours/uluru/16.jpg",
			"/Images/tours/uluru/17.jpg",
			"/Images/tours/uluru/18.jpg",
			"/Images/tours/uluru/19.jpg",
			"/Images/tours/uluru/20.jpg",
			"/Images/tours/uluru/21.jpg",
			"/Images/tours/uluru/22.jpg",
			"/Images/tours/uluru/23.jpg",
			"/Images/tours/uluru/24.jpg",
			"/Images/tours/uluru/25.jpg",
			"/Images/tours/uluru/26.jpg",
		],
		link: "https://www.netcodedev.com",
		price: "",
		rating: 4,
	},
	{
		id: "au10",
		title: "Blue Mountains - New South Wales",
		description: [
			"The Blue Mountains, located in New South Wales, Australia, is a unique and breathtaking natural landscape that offers a mix of natural beauty, rich culture, and adventurous activities. Here’s why you should consider visiting the Blue Mountains:",
			"The Blue Mountains offer a unique blend of natural beauty, rich culture, and adventurous activities. Whether you’re seeking a peaceful retreat in the mountains, exploring the stunning landscapes, or embarking on a thrilling adventure, the Blue Mountains has something for every traveler. So, pack your bags and get ready to explore the Blue Mountains and discover why it’s one of the world’s most beautiful and diverse destinations!",
		],
		imageUrl: [
			"/Images/tours/blue-mt/1.jpg",
			"/Images/tours/blue-mt/2.jpg",
			"/Images/tours/blue-mt/3.jpg",
			"/Images/tours/blue-mt/4.jpg",
			"/Images/tours/blue-mt/5.jpg",
			"/Images/tours/blue-mt/6.jpg",
			"/Images/tours/blue-mt/7.jpg",
			"/Images/tours/blue-mt/8.jpg",
			"/Images/tours/blue-mt/9.jpg",
			"/Images/tours/blue-mt/10.jpg",
			"/Images/tours/blue-mt/11.jpg",
			"/Images/tours/blue-mt/12.jpg",
			"/Images/tours/blue-mt/13.jpg",
			"/Images/tours/blue-mt/14.jpg",
			"/Images/tours/blue-mt/15.jpg",
			"/Images/tours/blue-mt/16.jpg",
			"/Images/tours/blue-mt/17.jpg",
			"/Images/tours/blue-mt/18.jpg",
			"/Images/tours/blue-mt/19.jpg",
			"/Images/tours/blue-mt/20.jpg",
			"/Images/tours/blue-mt/21.jpg",
			"/Images/tours/blue-mt/22.jpg",
			"/Images/tours/blue-mt/23.jpg",
			"/Images/tours/blue-mt/24.jpg",
		],
		link: "https://www.netcodedev.com",
		price: "$190/adult. Children < 8 years old $160",
		rating: 4,
	},
	{
		id: "au11",
		title: "Wollongong Beach - Kiama - Oyster Farm (Wine Tasting) ",
		description: [
			"Discover the perfect blend of stunning coastal landscapes, vibrant local culture, and exciting activities in Wollongong Beach and Kiama, New South Wales, Australia. These destinations have something for everyone, whether you want to relax on pristine beaches, explore dramatic cliffs, or immerse yourself in the local charm.",
			"Wollongong boasts diverse beaches such as North Wollongong Beach, offering family-friendly facilities, City Beach for urban convenience, and Austinmer Beach's relaxed atmosphere, perfect for snorkelling. Adventure seekers can enjoy skydiving, hang gliding at Bald Hill, or surfing on local waves. Cultural attractions include Nan Tien Temple, Wollongong Botanic Garden, and the Art Gallery.",
			"Kiama is known for natural wonders like Kiama Blowhole and Cathedral Rocks, scenic walks such as Kiama Coast Walk and Minnamurra Rainforest, and local charm with attractions like Kiama Lighthouse and Farmers’ Market. Accommodations range from seaside resorts to boutique stays and camping options.",
			"Wollongong Beach and Kiama offer a perfect combination of natural beauty, adventure, cultural experiences, and local charm. Whether you’re exploring the stunning coastlines, enjoying thrilling outdoor activities, or immersing yourself in the rich culture and history, these destinations promise an unforgettable experience. So pack your bags and get ready to discover the wonders of Wollongong Beach and Kiama in New South Wales!",
		],
		imageUrl: [
			"/Images/tours/wollongong-kiama/1.jpg",
			"/Images/tours/wollongong-kiama/2.jpg",
			"/Images/tours/wollongong-kiama/3.jpg",
			"/Images/tours/wollongong-kiama/4.jpg",
			"/Images/tours/wollongong-kiama/5.jpg",
			"/Images/tours/wollongong-kiama/6.jpg",
			"/Images/tours/wollongong-kiama/7.jpg",
			"/Images/tours/wollongong-kiama/8.jpg",
			"/Images/tours/wollongong-kiama/9.jpg",
			"/Images/tours/wollongong-kiama/10.jpg",
			"/Images/tours/wollongong-kiama/11.jpg",
			"/Images/tours/wollongong-kiama/12.jpg",
			"/Images/tours/wollongong-kiama/13.jpg",
			"/Images/tours/wollongong-kiama/14.jpg",
		],
		link: "https://www.netcodedev.com",
		price: "$195/person || Open every month || Departure from 10 guests.",
		rating: 4,
	},
	{
		id: "au12",
		title: "Camel Riding - Sandboarding - Dolphin Watching ",
		description: [],
		imageUrl: [
			"/Images/tours/camel-riding/1.jpg",
			"/Images/tours/camel-riding/2.jpg",
			"/Images/tours/camel-riding/3.jpg",
			"/Images/tours/camel-riding/4.jpg",
			"/Images/tours/camel-riding/5.jpg",
			"/Images/tours/camel-riding/6.jpg",
			"/Images/tours/camel-riding/7.jpg",
			"/Images/tours/camel-riding/8.jpg",
			"/Images/tours/camel-riding/9.jpg",
			"/Images/tours/camel-riding/10.jpg",
			"/Images/tours/camel-riding/11.jpg",
		],
		link: "https://www.netcodedev.com",
		price:
			"$190 - $250/person || Children < 6 years old do not ride camels || Open on weekends || Departure from 10 guests.",
		rating: 4,
	},
];
export const ToursAustraliaVi: TourType[] = [
	{
		id: "au1",
		title: "Khám Phá Thành phố Sydney",
		price: "$160/ khách. Khởi hành từ 10 khách.",
		description: [
			"Sydney, Úc, là một thành phố tuyệt vời được tôn vinh nhờ các địa danh mang tính biểu tượng, bến cảng ngoạn mục, văn hóa sôi động và các điểm tham quan đa dạng. Đây là lý do tại sao Sydney nên đứng đầu danh sách của bạn để có một chuyến phiêu lưu khó quên:",
			"Sydney tự hào có những địa danh mang tính biểu tượng như Nhà hát Opera Sydney và Cầu Cảng Sydney. Đường bờ biển có những bãi biển tuyệt đẹp như Bondi và Manly, cũng như những lối đi dạo ven biển đẹp như tranh vẽ. Thành phố có nền văn hóa thịnh vượng với các phòng trưng bày nghệ thuật, bảo tàng và các buổi biểu diễn trực tiếp . Bạn cũng sẽ bắt gặp những công viên và khu vườn tươi tốt như Vườn Bách thảo Hoàng gia và Công viên Centennial. Thêm vào đó là các điểm tham quan thú vị bao gồm: Sở thú Taronga và nhiều hoạt động ngoài trời đa dạng; các món ăn ngon, chợ hải sản và ẩm thực đa văn hóa; nhiều lễ hội và sự kiện khác nhau trong suốt cả năm; đồng thời nơi đây nổi tiếng với sự thân thiện của người dân địa phương và bầu không khí quốc tế.",
			"Cho dù bạn háo hức khám phá những thắng cảnh nổi tiếng thế giới, thư giãn trên những bãi biển tuyệt đẹp, đắm mình trong nghệ thuật hay đam mê các hoạt động nghệ thuật và đồ ăn ngon, Sydney có thứ gì đó dành cho mọi du khách. Vì vậy, hãy chuẩn bị trải nghiệm sự kỳ diệu của Sydney và tìm hiểu lý do tại sao đây là một trong những thành phố quyến rũ nhất thế giới!",
		],
		imageUrl: [
			"/Images/tours/sydney/1.jpg",
			"/Images/tours/sydney/2.jpg",
			"/Images/tours/sydney/3.jpg",
			"/Images/tours/sydney/4.jpg",
			"/Images/tours/sydney/5.jpg",
			"/Images/tours/sydney/6.jpg",
			"/Images/tours/sydney/7.jpg",
			"/Images/tours/sydney/8.jpg",
			"/Images/tours/sydney/9.jpg",
			"/Images/tours/sydney/10.jpg",
			"/Images/tours/sydney/11.jpg",
			"/Images/tours/sydney/12.jpg",
			"/Images/tours/sydney/13.jpg",
			"/Images/tours/sydney/14.jpg",
			"/Images/tours/sydney/15.jpg",
			"/Images/tours/sydney/16.jpg",
			"/Images/tours/sydney/17.jpg",
			"/Images/tours/sydney/18.jpg",
		],
		link: "https://www.netcodedev.com",
		rating: 4,
	},
	{
		id: "au2",
		title: "Melbourne",
		price: "",
		subTitle: [
			{
				id: "au2.1",
				title: "MÙA THU DANDENONG – YARRA VALLEY",
				price: "$150/khách, trẻ em dưới 10 tuổi vé $120",
				departs: "Khởi hành thứ 2, thứ 5 hàng tuần",
				schedule: [
					"🚗 Tàu cổ hơi nước – Vườn hoa Botanic Gardent – Thị trấn cổ -Thác nước – Xưởng Chocolate & ice cream.",
				],
			},
			{
				id: "au2.2",
				title: "MÙA THU BALLARAT",
				price: "$150/khách, trẻ em dưới 10 tuổi vé $120",
				departs: "Khởi hành thứ 3 hàng tuần",
				schedule: [
					"🚗 Hái trái cây – khu mỏ vàng – thị trấn Ballarat – Bảo tang nghệ thuật – vườn thực vật.",
				],
			},
			{
				id: "au2.3",
				title: "GREAT OCEAN ROAD",
				price: "$140/khách, trẻ em dưới 10 tuổi vé $110",
				departs: "Khởi hành hàng tuần",
				schedule: [
					"🚗 Vườn hồng, lâu đài cổ - 12 vị thánh tông đồ - Khe hẹp Loch Goerge – Appolo bay – Lorn –– Anglesea beach.",
					" *** (Nếu tour không đi vườn hồng và lâu đài cổ giá $120/ khách)",
					" *** Khách có nhu cầu đi trực thăng thì mua vé tự túc tại quầy giá $165/ người.",
				],
			},
			{
				id: "au2.4",
				title: "PENINSULAR",
				price: "$130/khách, trẻ em dưới 10 tuổi vé $100",
				departs: "Khởi hành thứ 2, thứ 7 hàng tuần",
				schedule: [
					"🚗 Cape Schanck Light house – Gape farm vườn thú Moonlit – Mornington beach.",
				],
			},
			{
				id: "au2.5",
				title: "BUFFLING BILLY RED HILL - PENINSULAR",
				price: "$160/khách, trẻ em dưới 10 tuổi vé $120",
				departs: "Khởi hành thứ 2, thứ 6 hàng tuần",
				schedule: [
					"🚗 Puffing Billy – Cape Schanck Lighthouse (Peninsula) – Gape farm – Botanic Gardent Dandenong.",
				],
			},
			{
				id: "au2.6",
				title: "WILSON PROMONTORY",
				price: "130/khách, trẻ em dưới 10 tuổi vé $100",
				departs: "Khởi hành chủ nhật hàng tuần",
				schedule: [
					"🚗 Wilson Prom – Tidal river – Norman Bay – Squeaky – Dunes of the Drift",
					" *** (Khách có nhu cầu đi tàu ra đảo thì mua vé tự túc giá $145/người).",
				],
			},
			{
				id: "au2.7",
				title: "LỄ HỘI MÙA THU MACEDON",
				price: "$150/adult, children < 10 years old $130",
				departs: "Khởi hành hàng tuần",
				schedule: [
					"🚗 Macedon – Vườn hoa Botanic Garden – Hanging Rock",
					" *** (Giá tour bao gồm vé vô vườn hoa Botanic Gardent và Hanging Rock).",
				],
			},
			{
				id: "au2.8",
				title: "MÙA THU BRIGHT",
				price: "$150/khách, trẻ em dưới 10 tuổi vé $130",
				departs: "Khởi hành hàng tuần",
				schedule: ["🚗 Bright – Ovens river – Lest we forget."],
			},
		],
		description: [
			"Melbourne, Úc, là một thành phố thú vị và sôi động, kết hợp hoàn hảo giữa văn hóa, vẻ đẹp tự nhiên và các điểm tham quan ly kỳ. Cho dù bạn đam mê nghệ thuật, ẩm thực, thiên nhiên hay phiêu lưu, Melbourne đều có thứ gì đó dành cho bạn. Dưới đây là lý do Melbourne nên là điểm đến phiêu lưu tiếp theo của bạn:",
			"Melbourne nổi tiếng với nền văn hóa và nghệ thuật đa dạng, tự hào với các phòng trưng bày nghệ thuật, nhà hát và địa điểm âm nhạc. Thành phố này là thiên đường ẩm thực với khung cảnh ẩm thực và cà phê thượng hạng, cung cấp nhiều lựa chọn ăn uống và văn hóa cà phê nổi tiếng. Hơn nữa, Melbourne còn tự hào có những công viên và khu vườn tuyệt đẹp, những điểm tham quan và hoạt động thú vị, bờ biển quyến rũ và những chuyến đi đẹp như tranh vẽ. Thành phố tổ chức các lễ hội và sự kiện sôi động quanh năm và có bầu không khí thân quen, mang tính quốc tế với người dân địa phương thân thiện và phương tiện giao thông công cộng thuận tiện.",
			"Melbourne là điểm đến kết hợp giữa sự phong phú về văn hóa, thú vui ẩm thực, vẻ đẹp tự nhiên và các hoạt động thú vị. Cho dù bạn muốn đắm mình trong nghệ thuật, thưởng thức ẩm thực đẳng cấp thế giới, khám phá cảnh quan tuyệt đẹp hay thưởng thức các lễ hội sôi động, Melbourne đều có thứ gì đó dành cho mọi du khách. Vì vậy, hãy  sẵn sàng khám phá Melbourne - một trong những thành phố năng động và quyến rũ nhất nước Úc!",
		],
		imageUrl: [
			"/Images/tours/melbourne/1.jpg",
			"/Images/tours/melbourne/2.jpg",
			"/Images/tours/melbourne/3.jpg",
			"/Images/tours/melbourne/4.jpg",
			"/Images/tours/melbourne/5.jpg",
			"/Images/tours/melbourne/6.jpg",
			"/Images/tours/melbourne/7.jpg",
			"/Images/tours/melbourne/8.jpg",
			"/Images/tours/melbourne/9.jpg",
			"/Images/tours/melbourne/10.jpg",
			"/Images/tours/melbourne/11.jpg",
			"/Images/tours/melbourne/12.jpg",
			"/Images/tours/melbourne/13.jpg",
			"/Images/tours/melbourne/14.jpg",
			"/Images/tours/melbourne/15.jpg",
			"/Images/tours/melbourne/16.jpg",
			"/Images/tours/melbourne/17.jpg",
			"/Images/tours/melbourne/18.jpg",
			"/Images/tours/melbourne/19.jpg",
		],
		link: "https://www.netcodedev.com",
		rating: 4,
	},
	{
		id: "au3",
		title: "Adelaide",
		description: [
			"Adelaide, thủ phủ thú vị của Nam Úc, thường không được chú ý nhiều mặc dù đây là một kho tàng vẻ đẹp tự nhiên, nền văn hóa phong phú và những trải nghiệm khó quên. Đây là lý do tại sao việc xem xét Adelaide cho chuyến phiêu lưu tiếp theo của bạn là một ý tưởng tuyệt vời:",
			"Adelaide tự hào có khung cảnh ngoạn mục và vẻ đẹp tự nhiên, bao gồm những bãi biển hoang sơ, cảnh quan tươi tốt và những khu vườn thanh bình. Thành phố ngập tràn văn hóa và di sản, có các phòng trưng bày nghệ thuật, bảo tàng và di tích lịch sử. Đây cũng là trung tâm sản xuất rượu vang hảo hạng và những trải nghiệm dành cho du khách sành ăn. Du khách có thể tham gia vào các điểm tham quan và hoạt động thú vị, như gặp gỡ động vật hoang dã, phiêu lưu ngoài trời và các lễ hội sôi động quanh năm. Adelaide là một thành phố thân thiện và thú vị để khám phá.",
			"Adelaide là điểm đến kết hợp hoàn hảo giữa vẻ đẹp tự nhiên, văn hóa phong phú, rượu vang đẳng cấp thế giới và các điểm tham quan thú vị. Cho dù bạn đang tìm kiếm sự thư giãn trên bãi biển, khám phá những vườn nho tươi tốt hay hòa mình vào các lễ hội sôi động, Adelaide đều mang lại điều gì đó cho mọi du khách. Vì vậy, hãy chuẩn bị cho hành trình khám phá lý do tại sao Adelaide nổi bật là một trong những điểm đến quyến rũ và đa dạng nhất nước Úc!",
		],
		imageUrl: [
			"/Images/tours/adelaide/1.jpg",
			"/Images/tours/adelaide/2.jpg",
			"/Images/tours/adelaide/3.jpg",
			"/Images/tours/adelaide/4.jpg",
			"/Images/tours/adelaide/5.jpg",
			"/Images/tours/adelaide/6.jpg",
			"/Images/tours/adelaide/7.jpg",
			"/Images/tours/adelaide/8.jpg",
			"/Images/tours/adelaide/9.jpg",
			"/Images/tours/adelaide/10.jpg",
			"/Images/tours/adelaide/11.jpg",
			"/Images/tours/adelaide/12.jpg",
			"/Images/tours/adelaide/13.jpg",
			"/Images/tours/adelaide/14.jpg",
			"/Images/tours/adelaide/15.jpg",
			"/Images/tours/adelaide/16.jpg",
			"/Images/tours/adelaide/17.jpg",
		],
		link: "https://www.netcodedev.com",
		price: "",
		rating: 4,
	},
	// {
	// 	id: "au4",
	// 	title: "Brisbane",
	// 	description: [
	// 		"Brisbane, the capital of Queensland, Australia, is a vibrant and dynamic city that offers a perfect blend of natural beauty, cultural richness, and exciting attractions. Here’s why you should consider Brisbane for your next adventure:",
	// 		"Brisbane offers stunning river views, lush green spaces, and the iconic Mount Coot-tha for panoramic city vistas. The city boasts a thriving cultural scene with museums, galleries, and the Queensland Cultural Centre. Visitors can enjoy thrilling activities such as the Story Bridge Adventure Climb and wildlife encounters at the Lone Pine Koala Sanctuary. The diverse food scene includes markets, night markets, and craft breweries. Brisbane hosts various festivals and events throughout the year, and its friendly locals and laid-back lifestyle make it a welcoming destination.",
	// 		"Brisbane is a destination that offers a perfect mix of natural beauty, rich culture, thrilling attractions, and a friendly atmosphere. Whether you’re seeking adventure, relaxation, or cultural experiences, Brisbane has something for everyone. So, get ready to explore this vibrant city and discover why Brisbane is one of Australia’s most exciting destinations!",
	// 	],
	// 	imageUrl: [
	// 		"/Images/contact/form.webp",
	// 		"/Images/Demo/2.jpeg",
	// 		"/Images/Demo/3.jpeg",
	// 		"/Images/Demo/4.jpeg",
	// 		"/Images/Demo/5.jpeg",
	// 		"/Images/Demo/6.jpeg",
	// 	],
	// 	link: "https://www.netcodedev.com",
	// 	price: "Adult: $120, Children < 10 years old:  $90",
	// 	rating: 4,
	// },
	{
		id: "au5",
		title: "Gold Coast - Brisbane",
		description: [
			"Gold Coast ở Queensland, Australia, là thiên đường của những nhà thám hiểm, mang đến sự kết hợp thú vị giữa vẻ đẹp thiên nhiên tuyệt đẹp, văn hóa sôi động và các điểm tham quan ly kỳ. Đây là lý do tại sao bạn nên chọn Gold Coast là điểm đến phiêu lưu tiếp theo của mình:",
			"Gold Coast có những bãi biển ngoạn mục, công viên giải trí ly kỳ và trải nghiệm động vật hoang dã đa dạng. Du khách có thể khám phá những khu rừng nhiệt đới tươi tốt của Hinterland, thưởng thức các môn thể thao dưới nước và tận hưởng cuộc sống về đêm sôi động. Khu vực này cũng tự hào có khung cảnh văn hóa phong phú, thức ăn ngon và rượu vang , đồng thời tổ chức nhiều lễ hội và sự kiện khác nhau quanh năm. Người dân địa phương thân thiện và bầu không khí thoải mái khiến Gold Coast trở thành điểm đến gần gũi với du khách.",
			"Gold Coast là điểm đến mang đến sự kết hợp hoàn hảo giữa vẻ đẹp tự nhiên, các điểm tham quan ly kỳ, văn hóa phong phú và cuộc sống về đêm sôi động. Cho dù bạn muốn thư giãn trên bãi biển, tìm kiếm các hoạt động kích thích cảm giác mạnh hay hòa mình vào văn hóa địa phương , Gold Coast luôn có điều gì đó dành cho du khách khám phá. Vì vậy, hãy sẵn sàng cho một chuyến phiêu lưu khó quên và khám phá lý do tại sao Gold Coast lại là một trong những điểm đến hàng đầu của Úc!",
		],
		imageUrl: [
			"/Images/tours/gold-coast/1.jpg",
			"/Images/tours/gold-coast/2.jpg",
			"/Images/tours/gold-coast/3.jpg",
			"/Images/tours/gold-coast/4.jpg",
			"/Images/tours/gold-coast/5.jpg",
			"/Images/tours/gold-coast/6.jpg",
			"/Images/tours/gold-coast/7.jpg",
			"/Images/tours/gold-coast/8.jpg",
			"/Images/tours/gold-coast/9.jpg",
			"/Images/tours/gold-coast/10.jpg",
			"/Images/tours/gold-coast/11.jpg",
			"/Images/tours/gold-coast/12.jpg",
			"/Images/tours/gold-coast/13.jpg",
			"/Images/tours/gold-coast/14.jpg",
			"/Images/tours/gold-coast/15.jpg",
			"/Images/tours/gold-coast/16.jpg",
			"/Images/tours/gold-coast/17.jpg",
			"/Images/tours/gold-coast/18.jpg",
			"/Images/tours/gold-coast/19.jpg",
			"/Images/tours/gold-coast/20.jpg",
			"/Images/tours/gold-coast/21.jpg",
			"/Images/tours/gold-coast/22.jpg",
			"/Images/tours/gold-coast/23.jpg",
			"/Images/tours/gold-coast/24.jpg",
			"/Images/tours/gold-coast/25.jpg",
			"/Images/tours/gold-coast/26.jpg",
			"/Images/tours/gold-coast/27.jpg",
			"/Images/tours/gold-coast/28.jpg",
			"/Images/tours/gold-coast/29.jpg",
			"/Images/tours/gold-coast/30.jpg",
			"/Images/tours/gold-coast/31.jpg",
			"/Images/tours/gold-coast/32.jpg",
		],
		link: "https://www.netcodedev.com",
		price: "",
		rating: 4,
	},
	// {
	// 	id: "au6",
	// 	title: "Great Barrier Reef",
	// 	description: [
	// 		"The Great Barrier Reef is the largest coral reef system in the world, offering mesmerizing natural beauty and vibrant marine life. It spans over 2,300 kilometers and comprises more than 2,900 individual reefs and 900 islands, providing excellent visibility for appreciating the coral formations and marine ecosystems. It is home to thousands of species, including over 1,500 species of fish, 400 types of coral, and majestic creatures such as manta rays, sea turtles, and dugongs. You can explore the reef through snorkeling, scuba diving, or glass-bottom boat tours, and discover its many islands, each with its own unique charm. The reef also offers opportunities for eco-friendly tours, luxury stays on islands, and engaging with the traditional owners of the reef. Additionally, it provides a plethora of water sports and relaxation options, and is easily accessible from major gateway cities like Cairns, Townsville, and Port Douglas.",
	// 		"The Great Barrier Reef is a destination that captivates with its extraordinary natural beauty, incredible marine life, and diverse range of activities. Whether you’re an avid adventurer, a marine enthusiast, or simply looking to relax in paradise, the Great Barrier Reef offers an unparalleled experience. Embark on a journey to this magnificent wonder and create memories that will last a lifetime!",
	// 	],
	// 	imageUrl: [
	// 		"/Images/contact/form.webp",
	// 		"/Images/Demo/2.jpeg",
	// 		"/Images/Demo/3.jpeg",
	// 		"/Images/Demo/4.jpeg",
	// 		"/Images/Demo/5.jpeg",
	// 		"/Images/Demo/6.jpeg",
	// 	],
	// 	link: "https://www.netcodedev.com",
	// 	price: "Adult: $120, Children < 10 years old:  $90",
	// 	rating: 4,
	// },
	{
		id: "au7",
		title: "Perth",
		description: [
			"Perth, Úc, là điểm đến đặc biệt cho bất kỳ du khách nào đang tìm kiếm sự kết hợp giữa vẻ đẹp tự nhiên, văn hóa sôi động và các điểm tham quan độc đáo. Đây là lý do bạn nên xách ba lô lên và đến thành phố tuyệt đẹp này:",
			"Perth, Australia có khung cảnh ngoạn mục và vẻ đẹp tự nhiên, với cơ hội tham gia các hoạt động dưới nước và cảnh hoàng hôn tuyệt đẹp tại các bãi biển như Cottesloe và Scarborough. Thành phố cũng tự hào có nền văn hóa phong phú, kết hợp di sản bản địa với cuộc sống hiện đại của Úc thông qua các khung cảnh nghệ thuật và văn hóa sôi động.",
			"Du khách có thể tận hưởng cuộc sống hoang dã tại Đảo Rottnest, các chuyến tham quan nếm rượu vang ở Thung lũng Swan, và các hoạt động kích thích cảm giác mạnh như nhảy dù và lướt sóng. Perth tổ chức nhiều lễ hội và sự kiện trong suốt cả năm, bao gồm Lễ hội Perth và Lễ hội Taste Great Southern. Bầu không khí thân thiện, thoải mái, sự mến khách và môi trường an toàn của thành phố khiến nơi đây trở thành một nơi lý tưởng để ghé thăm.",
			"Perth là điểm đến mang đến sự kết hợp hoàn hảo giữa vẻ đẹp tự nhiên, văn hóa phong phú, các điểm tham quan ly kỳ và bầu không khí thân thiện. Cho dù bạn muốn thư giãn trên bãi biển, khám phá cảnh quan đa dạng hay hòa mình vào văn hóa địa phương, Perth đều có điều gì đó dành cho mọi du khách. Vậy tại sao phải chờ đợi? Hãy bắt đầu lên kế hoạch cho chuyến phiêu lưu khó quên của bạn đến Perth ngay hôm nay!",
		],
		imageUrl: [
			"/Images/tours/perth/1.jpg",
			"/Images/tours/perth/2.jpg",
			"/Images/tours/perth/3.jpg",
			"/Images/tours/perth/4.jpg",
			"/Images/tours/perth/5.jpg",
			"/Images/tours/perth/6.jpg",
			"/Images/tours/perth/7.jpg",
			"/Images/tours/perth/8.jpg",
			"/Images/tours/perth/9.jpg",
		],
		link: "https://www.netcodedev.com",
		price: "",
		rating: 4,
	},
	{
		id: "au8",
		title: "Tasmania",
		description: [
			"Tasmania, quốc đảo của Úc, là một viên ngọc ẩn mang đến chuyến phiêu lưu tuyệt vời với vùng hoang sơ, lịch sử phong phú và nền văn hóa sôi động. Đây là lý do Tasmania nên là điểm đến du lịch tiếp theo của bạn:",
			"Tasmania có vẻ đẹp tự nhiên tuyệt đẹp, bao gồm các khu vực hoang dã Di sản Thế giới và các công viên quốc gia mang tính biểu tượng như Núi Cradle-Hồ St Clair và Freycinet. Hòn đảo này là nơi sinh sống của các loài động vật hoang dã độc đáo như quỷ Tasmania và tự hào có lịch sử và văn hóa phong phú. Ẩm thực bản địa có rất nhiều thú vị, nhà máy rượu vang nổi tiếng, và các sản phẩm thủ công. Du khách có thể tận hưởng việc đi bộ đường dài, chèo thuyền kayak, và ngắm động vật hoang dã khi hòn đảo tổ chức các lễ hội và sự kiện độc đáo như MONA FOMA và Taste của Tasmania. Người Tasmania được biết đến với sự hiếu khách và thân thiện, đồng thời các khu chợ địa phương là cách tuyệt vời để trải nghiệm những sản phẩm và hàng thủ công tuyệt vời nhất của Tasmania.",
			"Tasmania mang đến sự pha trộn độc đáo giữa vẻ đẹp thiên nhiên tuyệt đẹp, lịch sử phong phú, văn hóa sôi động và ẩm thực ngon miệng. Cho dù bạn là người đam mê hoạt động ngoài trời, người yêu thích lịch sử, người thích ẩm thực hay chỉ đơn giản là tìm kiếm một nơi thư giãn yên bình, Tasmania đều có thứ gì đó dành cho bạn.",
		],
		imageUrl: [
			"/Images/tours/tasmania/1.jpg",
			"/Images/tours/tasmania/2.jpg",
			"/Images/tours/tasmania/3.jpg",
			"/Images/tours/tasmania/4.jpg",
			"/Images/tours/tasmania/5.jpg",
			"/Images/tours/tasmania/6.jpg",
			"/Images/tours/tasmania/7.jpg",
			"/Images/tours/tasmania/8.jpg",
			"/Images/tours/tasmania/9.jpg",
			"/Images/tours/tasmania/10.jpg",
			"/Images/tours/tasmania/11.jpg",
			"/Images/tours/tasmania/12.jpg",
			"/Images/tours/tasmania/13.jpg",
		],
		link: "https://www.netcodedev.com",
		price: "",
		rating: 4,
	},
	{
		id: "au9",
		title: "ULURU- Núi Đá Đỏ",
		description: [
			"Uluru, còn được gọi là Ayers Rock, là một trong những địa danh tự nhiên mang tính biểu tượng và đầy cảm hứng nhất của Úc. Nằm ở trung tâm Lãnh thổ phía Bắc, khối đá sa thạch khổng lồ này có ý nghĩa văn hóa sâu sắc đối với người Anangu, những người bảo vệ truyền thống của vùng đất. Chuyến thăm Uluru mang đến sự kết hợp độc đáo giữa vẻ đẹp tự nhiên, nền văn hóa Thổ dân phong phú và những trải nghiệm khó quên. Đây là lý do bạn nên cân nhắc việc chọn Uluru là điểm thưởng ngoạn tiếp theo của mình:",
			"Uluru có đặc điểm địa chất độc đáo với độ cao 348m và sự thay đổi màu sắc ấn tượng. Uluru có ý nghĩa tâm linh đối với người Anangu và các chuyến tham quan văn hóa mang đến cái nhìn sâu sắc về những câu chuyện và truyền thống của Dreamtime. Du khách có thể trải nghiệm cảnh bình minh và hoàng hôn huyền diệu, chứng kiến ​​màu sắc rực rỡ của Cánh đồng ánh sáng sau khi trời tối và khám phá khu vực trên các chuyến tham quan bằng lạc đà. Sa mạc xung quanh Uluru là nơi có hệ động thực vật độc đáo, với nhiều cơ hội ngắm nhìn động vật hoang dã và ngắm chim. từ những khu nghỉ dưỡng sang trọng đến những lựa chọn vừa túi tiền, tất cả đều mang lại trải nghiệm văn hóa và ẩm thực cao cấp.",
			"Uluru mang đến trải nghiệm du lịch độc đáo và khó quên, kết hợp vẻ đẹp thiên nhiên ngoạn mục, văn hóa Thổ dân phong phú cùng hàng loạt hoạt động và phiêu lưu. Cho dù bạn đang ngắm bình minh trên tảng đá mang tính biểu tượng, khám phá nghệ thuật trên đá cổ xưa hay tìm hiểu về truyền thống của người Anangu, Uluru hứa hẹn sẽ là một hành trình kỳ thú. Vì vậy, hãy sẵn sàng khám phá một trong những điểm đến thiêng liêng và quý giá nhất của Úc, Uluru.",
		],
		imageUrl: [
			"/Images/tours/uluru/1.jpg",
			"/Images/tours/uluru/2.jpg",
			"/Images/tours/uluru/3.jpg",
			"/Images/tours/uluru/4.jpg",
			"/Images/tours/uluru/5.jpg",
			"/Images/tours/uluru/6.jpg",
			"/Images/tours/uluru/7.jpg",
			"/Images/tours/uluru/8.jpg",
			"/Images/tours/uluru/9.jpg",
			"/Images/tours/uluru/10.jpg",
			"/Images/tours/uluru/11.jpg",
			"/Images/tours/uluru/12.jpg",
			"/Images/tours/uluru/13.jpg",
			"/Images/tours/uluru/14.jpg",
			"/Images/tours/uluru/15.jpg",
			"/Images/tours/uluru/16.jpg",
			"/Images/tours/uluru/17.jpg",
			"/Images/tours/uluru/18.jpg",
			"/Images/tours/uluru/19.jpg",
			"/Images/tours/uluru/20.jpg",
			"/Images/tours/uluru/21.jpg",
			"/Images/tours/uluru/22.jpg",
			"/Images/tours/uluru/23.jpg",
			"/Images/tours/uluru/24.jpg",
			"/Images/tours/uluru/25.jpg",
			"/Images/tours/uluru/26.jpg",
		],
		link: "https://www.netcodedev.com",
		price: "",
		rating: 4,
	},
	{
		id: "au10",
		title: "Blue Mountains - New South Wales",
		description: [
			"Dãy Blue Mountains nằm ở New South Wales, Úc, là một cảnh quan thiên nhiên ngoạn mục và độc đáo, mang đến sự kết hợp giữa vẻ đẹp tự nhiên, văn hóa phong phú và các hoạt động mạo hiểm. Đây là lý do bạn nên cân nhắc đến thăm Blue Mountains:",
			"Dãy Blue Mountains mang đến sự kết hợp độc đáo giữa vẻ đẹp tự nhiên, văn hóa phong phú và các hoạt động mạo hiểm. Cho dù bạn đang tìm kiếm một nơi trú ẩn yên bình trên núi, khám phá những cảnh quan tuyệt đẹp hay hoà mình vào một cuộc phiêu lưu ly kỳ, Blue Mountains đều có thứ gì đó dành cho mọi du khách.",
			"Vì vậy, hãy sẵn sàng khám phá Blue Mountains để biết lý do tại sao nơi đây là một trong những điểm đến đẹp và đa dạng nhất thế giới!",
		],
		imageUrl: [
			"/Images/tours/blue-mt/1.jpg",
			"/Images/tours/blue-mt/2.jpg",
			"/Images/tours/blue-mt/3.jpg",
			"/Images/tours/blue-mt/4.jpg",
			"/Images/tours/blue-mt/5.jpg",
			"/Images/tours/blue-mt/6.jpg",
			"/Images/tours/blue-mt/7.jpg",
			"/Images/tours/blue-mt/8.jpg",
			"/Images/tours/blue-mt/9.jpg",
			"/Images/tours/blue-mt/10.jpg",
			"/Images/tours/blue-mt/11.jpg",
			"/Images/tours/blue-mt/12.jpg",
			"/Images/tours/blue-mt/13.jpg",
			"/Images/tours/blue-mt/14.jpg",
			"/Images/tours/blue-mt/15.jpg",
			"/Images/tours/blue-mt/16.jpg",
			"/Images/tours/blue-mt/17.jpg",
			"/Images/tours/blue-mt/18.jpg",
			"/Images/tours/blue-mt/19.jpg",
			"/Images/tours/blue-mt/20.jpg",
			"/Images/tours/blue-mt/21.jpg",
			"/Images/tours/blue-mt/22.jpg",
			"/Images/tours/blue-mt/23.jpg",
			"/Images/tours/blue-mt/24.jpg",
		],
		link: "https://www.netcodedev.com",
		price: "$190/khách. Trẻ em dưới 8 tuổi $160",
		rating: 4,
	},
	{
		id: "au11",
		title: "Biển Wollongong- Kiama- Trại hàu / Nếm thử rượu vang",
		description: [
			"Khám phá sự kết hợp hoàn hảo giữa cảnh quan ven biển tuyệt đẹp, văn hóa địa phương sôi động và các hoạt động thú vị ở Bãi biển Wollongong và Kiama, New South Wales, Australia. Những điểm đến này có điều gì đó dành cho tất cả du khách, cho dù bạn muốn thư giãn trên những bãi biển hoang sơ, khám phá những vách đá ấn tượng hay đắm mình trong sự quyến rũ ở nơi đây.",
			"Wollongong tự hào có những bãi biển đa dạng như Bãi biển Bắc Wollongong, cung cấp các tiện nghi thân thiện với gia đình, Bãi biển Thành phố để thuận tiện trong đô thị và bầu không khí thư giãn của Bãi biển Austinmer, thích hợp để lặn với ống thở. Những người tìm kiếm phiêu lưu có thể tận hưởng môn nhảy dù, lướt ván ở Bald Hill hoặc lướt trên những con sóng. Các điểm tham quan văn hóa bao gồm: Đền Nan Tien, Vườn Bách thảo Wollongong, và Phòng trưng bày Nghệ thuật.",
			"Kiama nổi tiếng với các kỳ quan thiên nhiên như Kiama Blowhole và Cathedral Rocks, những con đường đi bộ thơ mộng như Kiama Coast Walk và Rừng mưa nhiệt đới Minnamurra; cùng nét quyến rũ của các địa danh như Ngọn hải đăng Kiama và Chợ Nông sản. Kiama cung cấp loại hình lưu trú đa dạng, từ khu nghỉ dưỡng bên bờ biển đến nhà nghỉ kiểu boutique và lựa chọn cắm trại. ",
			"Bãi biển Wollongong và Kiama mang đến sự kết hợp hoàn hảo giữa vẻ đẹp tự nhiên, sự phiêu lưu, trải nghiệm văn hóa và nét quyến rũ của địa phương. Cho dù bạn đang khám phá những bờ biển tuyệt đẹp, tận hưởng các hoạt động ngoài trời ly kỳ hay đắm mình trong nền văn hóa và lịch sử phong phú, những điểm đến này hứa hẹn mang lại cho bạn một trải nghiệm tuyệt vời.",
		],
		imageUrl: [
			"/Images/tours/wollongong-kiama/1.jpg",
			"/Images/tours/wollongong-kiama/2.jpg",
			"/Images/tours/wollongong-kiama/3.jpg",
			"/Images/tours/wollongong-kiama/4.jpg",
			"/Images/tours/wollongong-kiama/5.jpg",
			"/Images/tours/wollongong-kiama/6.jpg",
			"/Images/tours/wollongong-kiama/7.jpg",
			"/Images/tours/wollongong-kiama/8.jpg",
			"/Images/tours/wollongong-kiama/9.jpg",
			"/Images/tours/wollongong-kiama/10.jpg",
			"/Images/tours/wollongong-kiama/11.jpg",
			"/Images/tours/wollongong-kiama/12.jpg",
			"/Images/tours/wollongong-kiama/13.jpg",
			"/Images/tours/wollongong-kiama/14.jpg",
		],
		link: "https://www.netcodedev.com",
		price: "$195/khách || Mở mỗi tháng || Khởi hành từ 10 khách.",
		rating: 4,
	},
	{
		id: "au12",
		title: "Cưỡi Cưỡi Lạc Đà - Trượt Cát - Ngắm Cá Heo",
		description: [],
		imageUrl: [
			"/Images/tours/camel-riding/1.jpg",
			"/Images/tours/camel-riding/2.jpg",
			"/Images/tours/camel-riding/3.jpg",
			"/Images/tours/camel-riding/4.jpg",
			"/Images/tours/camel-riding/5.jpg",
			"/Images/tours/camel-riding/6.jpg",
			"/Images/tours/camel-riding/7.jpg",
			"/Images/tours/camel-riding/8.jpg",
			"/Images/tours/camel-riding/9.jpg",
			"/Images/tours/camel-riding/10.jpg",
			"/Images/tours/camel-riding/11.jpg",
		],
		link: "https://www.netcodedev.com",
		price:
			"$190-$250/ khách || Trẻ em dưới 6 tuổi không cưỡi lạc đà || Mở các cuối tuần tour ghép thứ 7, cn  || Khởi hành từ 10 khách.",
		rating: 4,
	},
];
export const ToursOtherCountries: TourType[] = [
	{
		id: "vi1",
		title: "Across Vietnam",
		description: [
			"Vietnam is an extraordinary destination that promises a blend of breathtaking landscapes, rich cultural heritage, and vibrant city life. ",
			"Vietnamese cuisine is a highlight of any visit. From the world-famous pho (noodle soup) to fresh spring rolls and banh mi (Vietnamese sandwich), the flavors are vibrant and unforgettable. Don’t miss out on street food tours to experience the authentic taste of Vietnam.",
			"The people of Vietnam are known for their hospitality and friendliness. Whether you’re in a bustling city or a remote village, you’ll find locals eager to share their culture and traditions with you.For thrill-seekers, Vietnam offers a range of activities such as kayaking in Ha Long Bay, motorbiking along the Hai Van Pass, and trekking in the central highlands. The country’s diverse landscape provides endless opportunities for outdoor adventures.",
			"Vietnam is an extraordinary destination known for its stunning natural beauty, rich cultural heritage, exciting attractions, delicious cuisine, warm and welcoming people, and adventure opportunities. The country offers breathtaking landscapes, vibrant city life, and a blend of old and new in places like Hanoi and Ho Chi Minh City. With its UNESCO World Heritage sites, stunning natural wonders like Ha Long Bay and Phong Nha-Ke Bang National Park, and delicious cuisine, Vietnam promises an unforgettable adventure for travellers.",
		],
		imageUrl: [
			"/Images/tours/vietnam/1.jpg",
			"/Images/tours/vietnam/2.jpg",
			"/Images/tours/vietnam/3.jpg",
			"/Images/tours/vietnam/4.jpg",
			"/Images/tours/vietnam/5.jpg",
			"/Images/tours/vietnam/6.jpg",
			"/Images/tours/vietnam/7.jpg",
			"/Images/tours/vietnam/8.jpg",
			"/Images/tours/vietnam/9.jpg",
			"/Images/tours/vietnam/10.jpg",
			"/Images/tours/vietnam/11.jpg",
			"/Images/tours/vietnam/12.jpg",
			"/Images/tours/vietnam/13.jpg",
			"/Images/tours/vietnam/14.jpg",
			"/Images/tours/vietnam/15.jpg",
			"/Images/tours/vietnam/16.jpg",
			"/Images/tours/vietnam/17.jpg",
			"/Images/tours/vietnam/18.jpg",
		],
		link: "https://www.netcodedev.com",
		price: "",
		rating: 4,
	},
	{
		id: "ba2",
		title: "Bali - Indonesia",
		description: [
			"Bali, Indonesia, is a mesmerizing tropical paradise offering a perfect combination of breathtaking natural beauty, a rich cultural heritage, and thrilling adventures. Here's why Bali should be your next travel destination:",
			"Renowned for its stunning natural beauty, Bali features picturesque beaches, lush rice terraces, and majestic volcanic mountains. The island also showcases a rich culture deeply intertwined with Hinduism, vibrant dance and music scenes, and festivals that provide an insight into the island's spiritual life. Bali offers a wide array of outdoor activities, such as snorkelling, diving, trekking, and rafting. It's also a well-known destination for wellness and relaxation, featuring luxurious spas, yoga retreats, and holistic healing practices. Bali's cuisine is a delightful fusion of traditional Indonesian flavours and international influences, and the island provides unique shopping experiences with a wide variety of handmade items. Finally, the Balinese people are renowned for their friendliness and warm hospitality, ensuring an unforgettable travel experience.",
			"Bali is a destination that offers an ideal mix of natural beauty, cultural richness, adventurous activities, and wellness experiences. Whether you seek relaxation on beautiful beaches, exploration of lush landscapes, immersion in local culture, or thrilling adventures, Bali has something for every traveller. So, pack your bags and get ready to discover the magic of Bali and understand why it's one of the world's most beloved destinations!",
		],
		imageUrl: [
			"/Images/tours/bali/1.jpg",
			"/Images/tours/bali/4.jpg",
			"/Images/tours/bali/2.jpg",
			"/Images/tours/bali/3.jpg",
			"/Images/tours/bali/5.jpg",
			"/Images/tours/bali/6.jpg",
		],
		link: "https://www.netcodedev.com",
		price: "",
		rating: 4,
	},
	// {
	// 	id: "th3",
	// 	title: "Thailand",
	// 	description: [
	// 		"Thailand is a fascinating place that offers a wonderful combination of breathtaking scenery, deep-rooted culture, historical marvels, and bustling city life. Here's why Thailand deserves a top spot on your travel bucket list:",
	// 		"Thailand is renowned for its enchanting beaches, diverse cultural heritage, lively urban environment, delectable cuisine, thrilling escapades, wellness and relaxation opportunities, vibrant festivals, and the friendly nature of its people. The southern coastline boasts stunning beaches such as Phuket and the Phi Phi Islands, while Bangkok presents a fusion of contemporary skyscrapers and ancient temples. The cuisine encompasses everything from tantalizing street food to upscale dining, featuring specialties like pad Thai and green curry. Adventurous souls can explore mountainous regions, engage in world-class diving, or unwind at wellness resorts. Festivals like Songkran and Loy Krathong provide unique cultural experiences. Throughout the country, welcoming locals fosters a hospitable atmosphere for visitors.",
	// 		"Thailand is a destination that delivers a perfect blend of natural splendour, deep-rooted culture, bustling city life, and exhilarating adventures. Whether you seek to unwind on stunning beaches, explore ancient temples, savour delectable cuisine, or partake in thrilling activities, Thailand offers something for every traveller. So, prepare your bags and get ready to soak in the enchantment of Thailand to understand why it's one of the world's most adored travel destinations!",
	// 	],
	// 	imageUrl: [
	// 		"/Images/tours/vietnam/6.jpg",
	// 		"/Images/tours/vietnam/1.jpg",
	// 		"/Images/tours/vietnam/4.jpg",
	// 		"/Images/tours/vietnam/2.jpg",
	// 		"/Images/tours/vietnam/3.jpg",
	// 		"/Images/tours/vietnam/5.jpg",
	// 		"/Images/tours/vietnam/7.jpg",
	// 		"/Images/tours/vietnam/8.jpg",
	// 		"/Images/tours/vietnam/9.jpg",
	// 		"/Images/tours/vietnam/10.jpg",
	// 	],
	// 	link: "https://www.netcodedev.com",
	// 	price: "Adult: $120, Children < 10 years old:  $90",
	// 	rating: 4,
	// },
	{
		id: "ja4",
		title: "Japan - (04/2025 - 11/2025) ",
		subTitle: [
			{
				id: "ja4.1",
				title: "JAPAN TOUR SCHEDULE",
				price:
					" AUD$4,800/person. Children under 8 years old: AUD$3,700, children under 2 years old: AUD$2,100/child.(booking after January 15, 2025, tour price will add 20%)",
				departs: "November 2024 and April 2025",
				schedule: [
					"Tour includes: ",
					"* Round trip air tickets Sydney - Japan,",
					"* Have breakfast and dinner in the group (lunch on your own at the culinary streets with the guidance of the tour guide). The whole group will be given a party-style meal at the restaurant if the group has more than 15 guests.",
					"* Drinking water throughout the journey,",
					"* Accompanying tour guide from Australia and throughout the route. Local tour guide living in Japan,",
					"* Tickets to all attractions in the program,",
					"* 3-5 star standard hotel (2 people per room or shared room for family). If you stay in a private room for 1 person, an additional charge of AUD$400/person will be added.",
					"* Experience all the best features of 5 famous prefectures in Japan,",
					"* Shinkansen bullet train ticket,",
					"* Shuttle bus in Japan and airport shuttle in Japan,",
					"",
					"* ***Tip for Tour Guide: AUD$10/day/person for groups of over 10 guests, or AUD$15/day/person for groups of less than 10 guests. ",
					"* Souvenirs from Japanese localities for guests. ",
					"",
					"DAY 1 (March 20): FLY FROM SYDNEY TO TOKYO",
					"Pick up at the airport to the hotel at 8-10pm. ",
					"",
					"DAY 2 (March 21): NARITA - TOKYO",
					"* The group enters the country, the car picks up the group at the hotel. ",
					"* Visit the banks of the Meguro River where you can admire the most beautiful British Island flowers in Tokyo ",
					"* Having lunch",
					"* Visit the Royal Palace where the King lives. ",
					"* Visit Odaiba Bay and check in to the Statue of Liberty. ",
					"* Check in hotel, rest and have dinner.",
					"",
					"DAY 3 (March 22): FUJI - FUJI ",
					"* The group departs for Fuji. ",
					"* Visit Oshino Hakkai ancient village",
					"* Having lunch",
					"* Visit the 5-storey Chureito tower",
					"* Visit and walk around cherry blossoms at Lake Kawaguchi",
					"* Check in hotel, rest and have dinner.",
					"",
					"DAY 4 (March 23): HAKONE",
					"* The group departs for Hakone",
					"* Visit the steaming volcanic valley of Owakudani, eat 7-year-old black eggs",
					"* Visit and experience the Pirate ship on Lake Ashi",
					"* Having lunch",
					"* Depart for Osaka",
					"* Check in hotel, rest and have dinner.",
					"",
					"DAY 5 (March 24): KYOTO",
					"* The group departs for Kyoto",
					"* Visit the Golden Pagoda (Koyto symbol)",
					"* Having lunch",
					"* Visit and experience the streets of Kyoto",
					"* Gion Street.",
					"* Nishiki seafood market",
					"* Visit the bamboo forest and Arashiyama tourist area",
					"* Check in to the hotel to rest. ",
					"* Have dinner.",
					"",
					"DAY 6 (March 25): NARA - OSAKA",
					"* The group departs for Nara",
					"* Visit Nara Park and Todaji temple",
					"* Having lunch",
					"* The group travels to central Osaka to shop and explore famous streets: Dotonbori, Namba, Shinsaibashi.",
					"* Check in to the hotel, rest and have dinner.",
					"",
					"* DAY 7 (March 26): OSAKA - TOKYO",
					"* The group checks out of the hotel",
					"* Visit Osaka Castle ",
					"* Having lunch ",
					"* The group travels to Tokyo by Shikansen (SHINOSAKA-TOKYO)",
					"* Have dinner ",
					"* The group got picked up at Tokyo station and seen off at Narita airport.",
					"DAY 7 (March 27): RETURNS AT SYDNEY AIRPORT.",
					"",
					"*** Customers please read the itinerary carefully and transfer money to reserve their seats immediately AUD$2,500/person x the total number of guests. ",
					"Or visit the representative office in Cabramatta - Address: 11 Burradoo Road, Lansvale, NSW 2166",
					"Contact Ann via phone number 0401 211 890., please contact 1 day before your arrival for Ann’s availability.",
				],
			},
		],
		description: [
			"Japan is a captivating blend of ancient traditions and modern innovation. It offers a wide range of experiences, from peaceful temples and verdant gardens to lively cities and cutting-edge technology. Here's why Japan should be your next travel destination:",
			"Japan's rich cultural heritage spans millennia, with historical landmarks such as Kyoto's Kinkaku-ji (Golden Pavilion) and Nara's Todai-ji Temple, home to the world's largest bronze Buddha statue. Tokyo seamlessly combines history and modernity in places like the Asakusa district and Senso-ji temple. Natural beauty abounds, from the majestic presence of Mount Fuji to the cherry blossom season (sakura) and vibrant fall foliage (koyo) in Nikko and the Japanese Alps.",
			"Urban adventures await with Tokyo's juxtaposition of traditional temples and neon-lit streets, and Osaka's vibrant culinary scene in the Dotonborn district. Indulge in world-renowned cuisine such as sushi, ramen, and kaiseki in Kyoto. Unique cultural experiences include traditional tea ceremonies, sumo wrestling matches, kabuki theatre performances, and futuristic attractions like the teamLab Borderless museum in Tokyo.",
			"Japanese festivals honour traditions with events like the Gion Matsuri in July and the Sapporo Snow Festival in winter. The country's hospitality, rooted in omotenashi, ensures that visitors feel welcome in a variety of accommodations.",
			"Japan is a destination that offers an extraordinary mix of ancient traditions, stunning natural landscapes, cutting-edge technology, and unique cultural experiences. Whether you're exploring historic temples, enjoying exquisite cuisine, embarking on urban adventures, or participating in traditional festivals, Japan has something to captivate every traveller. So, pack your bags and get ready to embark on an unforgettable journey to the Land of the Rising Sun!",
		],
		imageUrl: [
			"/Images/tours/japan/1.jpg",
			"/Images/tours/japan/2.jpg",
			"/Images/tours/japan/3.jpg",
			"/Images/tours/japan/4.jpg",
			"/Images/tours/japan/5.jpg",
			"/Images/tours/japan/6.jpg",
			"/Images/tours/japan/7.jpg",
			"/Images/tours/japan/8.jpg",
			"/Images/tours/japan/9.jpg",
			"/Images/tours/japan/10.jpg",
			"/Images/tours/japan/11.jpg",
			"/Images/tours/japan/12.jpg",
			"/Images/tours/japan/13.jpg",
			"/Images/tours/japan/14.jpg",
			"/Images/tours/japan/15.jpg",
			"/Images/tours/japan/16.jpg",
			"/Images/tours/japan/17.jpg",
			"/Images/tours/japan/18.jpg",
			"/Images/tours/japan/19.jpg",
			"/Images/tours/japan/20.jpg",
			"/Images/tours/japan/21.jpg",
			"/Images/tours/japan/22.jpg",
		],
		link: "https://www.netcodedev.com",
		price: "",
		rating: 4,
	},
	// {
	// 	id: "ko5",
	// 	title: "South Korea",
	// 	description: [
	// 		"South Korea is an energetic and diverse country that provides a distinctive mix of age-old customs, contemporary innovations, stunning natural beauty, and thrilling urban adventures. Here's why you should consider South Korea for your next travel destination:",
	// 		"South Korea boasts a rich cultural legacy, including UNESCO World Heritage sites such as Changdeokgung Palace and Gyeongbokgung Palace. Take in the traditional lifestyle at Bukchon Hanok Village with its well-preserved hanok houses. In Seoul, ancient palaces and temples harmoniously coexist with modern skyscrapers and futuristic structures in areas like Gangnam and Hongdae.",
	// 		"Busan offers gorgeous beaches, vibrant seafood markets, and cultural festivities at Haeundae Beach and Gwangalli Beach. The lively atmosphere of Jagalchi Fish Market, where fresh seafood is abundant, is a must-visit. South Korea's natural beauty encompasses Hallasan Mountain on Jeju Island and the rugged mountains and forests of Seoraksan National Park.",
	// 		"Korean cuisine boasts bold flavors, diverse dishes, and a focus on fresh ingredients, such as bibimbap, bulgogi, and kimchi. In Seoul, you can savor street foods like tteokbokki, hotteok, and kimbap. Busan is renowned for its seafood, particularly sashimi and haemul jjigae.",
	// 		"One-of-a-kind cultural experiences in South Korea include tea ceremonies, staying in hanoks, visiting taekwondo gyms, enjoying pansori performances and samulnori music, and partaking in vibrant festivals like the Lantern Festival, Boryeong Mud Festival, Jinhae Cherry Blossom Festival, and Andong Mask Dance Festival.",
	// 		"South Korea's vibrant pop culture encompasses K-pop, K-dramas, fashion, and trendy neighborhoods like Myeongdong and Dongdaemun. The transportation system is efficient, with high-speed trains and subways in major cities. South Koreans are known for their warm hospitality and the concept of jeong.",
	// 		"South Korea is a destination that offers an ideal combination of age-old traditions, modern innovations, breathtaking natural landscapes, and vibrant urban experiences. Whether you're exploring historic palaces, relishing delicious cuisine, partaking in cultural festivals, or immersing yourself in pop culture, South Korea has something for every traveller. So, get your bags packed, and get ready to delve into the marvels of South Korea – truly one of the world's most enthralling travel destinations!",
	// 	],
	// 	imageUrl: [
	// 		"/Images/tours/vietnam/8.jpg",
	// 		"/Images/tours/vietnam/4.jpg",
	// 		"/Images/tours/vietnam/5.jpg",
	// 		"/Images/tours/vietnam/6.jpg",
	// 		"/Images/tours/vietnam/1.jpg",
	// 		"/Images/tours/vietnam/2.jpg",
	// 		"/Images/tours/vietnam/3.jpg",
	// 		"/Images/tours/vietnam/7.jpg",
	// 		"/Images/tours/vietnam/9.jpg",
	// 		"/Images/tours/vietnam/10.jpg",
	// 	],
	// 	link: "https://www.netcodedev.com",
	// 	price: "Adult: $120, Children < 10 years old:  $90",
	// 	rating: 4,
	// },
	// {
	// 	id: "us6",
	// 	title: "The United States",
	// 	description: [
	// 		"The USA is an extensive and diverse country that offers a variety of experiences, from awe-inspiring natural landscapes and lively cities to a rich cultural heritage and innovative attractions. Here's why a trip to America should be next on your travel agenda:",
	// 		"The United States boasts stunning natural wonders such as the Grand Canyon and Yellowstone National Park. Its vibrant cities like New York City and Los Angeles are renowned for their rich cultural scenes and entertainment options. The country holds a significant cultural legacy, with iconic landmarks in Washington, D.C., and musical hotspots in New Orleans. Additionally, it provides thrilling outdoor activities in places like the Rocky Mountains and the Florida Keys. The diverse American cuisine and unique cultural experiences, from Broadway shows in New York to live sports events, further contribute to the country's appeal. Moreover, the United States hosts a wide range of festivals and events, and visitors can anticipate encountering welcoming locals and experiencing warm hospitality throughout the country.",
	// 		"The United States is a destination that offers an extraordinary blend of natural beauty, lively cities, significant cultural heritage, and thrilling adventures. Whether you're exploring iconic landmarks, savoring delightful cuisine, enjoying cultural festivals, or embarking on outdoor adventures, the United States has something to captivate every traveller. So, pack your bags and get ready to uncover the wonders of America and discover why it's one of the world's most diverse and dynamic travel destinations!",
	// 	],
	// 	imageUrl: [
	// 		"/Images/tours/vietnam/7.jpg",
	// 		"/Images/tours/vietnam/10.jpg",
	// 		"/Images/tours/vietnam/8.jpg",
	// 		"/Images/tours/vietnam/4.jpg",
	// 		"/Images/tours/vietnam/5.jpg",
	// 		"/Images/tours/vietnam/6.jpg",
	// 		"/Images/tours/vietnam/1.jpg",
	// 		"/Images/tours/vietnam/2.jpg",
	// 		"/Images/tours/vietnam/3.jpg",
	// 		"/Images/tours/vietnam/9.jpg",
	// 	],
	// 	link: "https://www.netcodedev.com",
	// 	price: "Adult: $120, Children < 10 years old:  $90",
	// 	rating: 4,
	// },
	// {
	// 	id: "ur7",
	// 	title: "Europe",
	// 	description: [
	// 		"Europe boasts a rich history, diverse culture, and stunning natural beauty, making it an ideal destination for all types of travellers. From ancient landmarks and charming villages to vibrant cities and breathtaking landscapes, Europe has it all. Here’s why you should consider Europe for your next travel adventure:",
	// 		"Europe is home to iconic historical landmarks such as the Eiffel Tower, Notre Dame Cathedral, and the Colosseum in Italy. The continent's vibrant cities and culture are exemplified in London, Barcelona, and Amsterdam, while its stunning natural landscapes include the Swiss Alps, Norwegian fjords, and the Amalfi Coast.",
	// 		"European cuisine encompasses diverse dishes such as coq au vin in France, pasta and pizza in Italy, tapas and paella in Spain, and traditional English breakfast in the UK. Unique cultural experiences include Oktoberfest in Germany, the Carnival of Venice, and Christmas markets in Austria and Germany.",
	// 		"Europe also has a rich artistic and musical heritage, with notable galleries and museums in Madrid, Florence, and Paris. The continent hosts a wide range of festivals and events, including the Edinburgh Festival Fringe, La Tomatina in Spain, the Running of the Bulls in Pamplona, and the Cannes Film Festival in France.",
	// 		"Europe offers an extraordinary blend of historical landmarks, vibrant cities, stunning natural landscapes, delicious cuisine, unique cultural experiences, and rich artistic heritage. Whether you’re exploring ancient ruins, indulging in gourmet meals, enjoying cultural festivals, or embarking on outdoor adventures, Europe has something to captivate every traveller. So, pack your bags and get ready to discover the wonders of Europe and why it’s one of the world’s most enchanting travel destinations!",
	// 	],
	// 	imageUrl: [
	// 		"/Images/tours/vietnam/2.jpg",
	// 		"/Images/tours/vietnam/6.jpg",
	// 		"/Images/tours/vietnam/7.jpg",
	// 		"/Images/tours/vietnam/10.jpg",
	// 		"/Images/tours/vietnam/8.jpg",
	// 		"/Images/tours/vietnam/4.jpg",
	// 		"/Images/tours/vietnam/5.jpg",
	// 		"/Images/tours/vietnam/1.jpg",
	// 		"/Images/tours/vietnam/3.jpg",
	// 		"/Images/tours/vietnam/9.jpg",
	// 	],
	// 	link: "https://www.netcodedev.com",
	// 	price: "Adult: $120, Children < 10 years old:  $90",
	// 	rating: 4,
	// },
	// {
	// 	id: "nz8",
	// 	title: "New Zealand",
	// 	description: [
	// 		"New Zealand offers an unparalleled blend of natural beauty, rich Maori culture, and exciting adventures. This South Pacific gem provides diverse experiences, from tranquil landscapes and vibrant cities to unique wildlife and outdoor activities.",
	// 		"New Zealand boasts stunning landscapes, from snow-capped mountains to lush rainforests. Must-visit spots include Milford Sound in Fiordland National Park, Aoraki/Mount Cook for hiking, and Rotorua's geothermal marvels like Wai-O-Tapu Thermal Wonderland.",
	// 		"New Zealand boasts stunning landscapes, from snow-capped mountains to lush rainforests. Must-visit spots include Milford Sound in Fiordland National Park, Aoraki/Mount Cook for hiking, and Rotorua's geothermal marvels like Wai-O-Tapu Thermal Wonderland.",
	// 		"Maori culture is deeply rooted in New Zealand, with Te Puia showcasing traditional arts and Waitangi Treaty Grounds offering historical insights. Immersive cultural experiences can be had through Marae’s visits.",
	// 		"Adventure enthusiasts can skydive over Queenstown, bungee jump at Kawarau Bridge, or trek the breathtaking Tongariro Alpine Crossing. Wildlife lovers can spot unique species such as kiwi birds and marine life in Kaikoura.",
	// 		"Charming cities like Auckland, Wellington, and Queenstown effortlessly blend modern amenities with local culture. New Zealand's cuisine features fresh seafood and farm-to-table dining, paired with world-class wines from regions like Marlborough and Central Otago.",
	// 		"Kiwis are famous for their warm hospitality, ensuring a welcoming experience in various accommodations across the country. In essence, New Zealand offers an extraordinary mix of natural beauty, rich culture, thrilling adventures, unique wildlife, and captivating cities. Whether you're exploring breathtaking landscapes, immersing yourself in Maori culture, enjoying outdoor activities, or savouring delicious cuisine and wine, New Zealand has something to captivate every traveller. So, pack your bags and get ready to discover the wonders of New Zealand, one of the world's most enchanting travel destinations!",
	// 	],
	// 	imageUrl: [
	// 		"/Images/tours/vietnam/2.jpg",
	// 		"/Images/tours/vietnam/6.jpg",
	// 		"/Images/tours/vietnam/7.jpg",
	// 		"/Images/tours/vietnam/10.jpg",
	// 		"/Images/tours/vietnam/8.jpg",
	// 		"/Images/tours/vietnam/4.jpg",
	// 		"/Images/tours/vietnam/5.jpg",
	// 		"/Images/tours/vietnam/1.jpg",
	// 		"/Images/tours/vietnam/3.jpg",
	// 		"/Images/tours/vietnam/9.jpg",
	// 	],
	// 	link: "https://www.netcodedev.com",
	// 	price: "Adult: $120, Children < 10 years old:  $90",
	// 	rating: 4,
	// },
];
export const ToursOtherCountriesVi: TourType[] = [
	{
		id: "vi1",
		title: "Tour Xuyên Việt - Việt Nam",
		description: [
			"Việt Nam là một điểm đến đặc biệt hứa hẹn sự kết hợp giữa cảnh quan ngoạn mục, di sản văn hóa phong phú và cuộc sống thành phố sôi động.",
			"Ẩm thực Việt Nam là điểm nhấn của bất kỳ chuyến du lịch nào. Từ món phở nổi tiếng thế giới đến chả giò tươi và bánh mì (bánh mì Việt Nam), hương vị đậm đà và khó quên. Đừng bỏ lỡ các tour ẩm thực đường phố để trải nghiệm hương vị đích thực của Việt Nam.",
			"Người dân Việt Nam nổi tiếng với lòng hiếu khách và sự thân thiện. Dù bạn đang ở một thành phố nhộn nhịp hay một ngôi làng xa xôi, bạn sẽ thấy người dân địa phương sẵn sàng chia sẻ văn hóa và truyền thống của họ với bạn. Đối với những người thích cảm giác mạnh, Việt Nam có nhiều lựa chọn các hoạt động như chèo thuyền kayak ở Vịnh Hạ Long, đi xe máy dọc đèo Hải Vân và đi bộ xuyên rừng ở vùng cao nguyên miền Trung mang đến cơ hội vô tận cho những chuyến phiêu lưu ngoài trời.",
			"Việt Nam là một điểm đến đặc biệt được biết đến với vẻ đẹp thiên nhiên tuyệt đẹp, di sản văn hóa phong phú, các điểm tham quan thú vị, ẩm thực ngon, con người nồng hậu và thân thiện cũng như các cơ hội phiêu lưu. Đất nước này có cảnh quan ngoạn mục, cuộc sống thành phố sôi động và sự pha trộn giữa cái cũ và cái mới trong những địa điểm như Hà Nội và Thành phố Hồ Chí Minh, với các Di sản Thế giới được UNESCO công nhận, các kỳ quan thiên nhiên tuyệt đẹp như Vịnh Hạ Long và Vườn quốc gia Phong Nha-Kẻ Bàng cùng ẩm thực ngon, Việt Nam hứa hẹn một chuyến phiêu lưu khó quên cho du khách.",
		],
		imageUrl: [
			"/Images/tours/vietnam/1.jpg",
			"/Images/tours/vietnam/2.jpg",
			"/Images/tours/vietnam/3.jpg",
			"/Images/tours/vietnam/4.jpg",
			"/Images/tours/vietnam/5.jpg",
			"/Images/tours/vietnam/6.jpg",
			"/Images/tours/vietnam/7.jpg",
			"/Images/tours/vietnam/8.jpg",
			"/Images/tours/vietnam/9.jpg",
			"/Images/tours/vietnam/10.jpg",
			"/Images/tours/vietnam/11.jpg",
			"/Images/tours/vietnam/12.jpg",
			"/Images/tours/vietnam/13.jpg",
			"/Images/tours/vietnam/14.jpg",
			"/Images/tours/vietnam/15.jpg",
			"/Images/tours/vietnam/16.jpg",
			"/Images/tours/vietnam/17.jpg",
			"/Images/tours/vietnam/18.jpg",
		],
		link: "https://www.netcodedev.com",
		price: "",
		rating: 4,
	},
	{
		id: "ba2",
		title: "Bali - Indonesia",
		description: [
			"Bali, Indonesia, là thiên đường nhiệt đới đầy mê hoặc mang đến sự kết hợp hoàn hảo giữa vẻ đẹp thiên nhiên ngoạn mục, di sản văn hóa phong phú và những cuộc phiêu lưu ly kỳ. Đây là lý do Bali nên là điểm đến du lịch tiếp theo của bạn:",
			"Nổi tiếng với vẻ đẹp thiên nhiên tuyệt đẹp, Bali có những bãi biển đẹp như tranh vẽ, ruộng bậc thang tươi tốt và những ngọn núi lửa hùng vĩ. Hòn đảo này còn thể hiện một nền văn hóa phong phú gắn bó sâu sắc với Ấn Độ giáo, những cảnh khiêu vũ và âm nhạc sôi động cũng như các lễ hội mang đến cái nhìn sâu sắc về tâm linh của hòn đảo. cuộc sống Bali cung cấp một loạt các hoạt động ngoài trời, chẳng hạn như lặn với ống thở, lặn, leo núi và đi bè. Đây cũng là điểm đến nổi tiếng về chăm sóc sức khỏe và thư giãn, có các spa sang trọng, các khóa tập yoga và ẩm thực toàn diện của Bali. sự kết hợp thú vị giữa hương vị truyền thống của Indonesia và ảnh hưởng quốc tế, đồng thời hòn đảo mang đến trải nghiệm mua sắm độc đáo với nhiều loại mặt hàng thủ công. Cuối cùng, người dân Bali nổi tiếng vì sự thân thiện và lòng hiếu khách nồng hậu, đảm bảo trải nghiệm du lịch khó quên.",
			"Bali là điểm đến mang đến sự kết hợp lý tưởng giữa vẻ đẹp tự nhiên, sự phong phú về văn hóa, các hoạt động mạo hiểm và trải nghiệm chăm sóc sức khỏe. Cho dù bạn tìm kiếm sự thư giãn trên những bãi biển xinh đẹp, khám phá những cảnh quan tươi tốt, hòa mình vào văn hóa địa phương hay những cuộc phiêu lưu kỳ thú, Bali đều có điều gì đó dành cho bạn Vì vậy, hãy đóng gói hành lý của bạn và sẵn sàng khám phá sự kỳ diệu của Bali và hiểu lý do tại sao nơi đây là một trong những điểm đến được yêu thích nhất trên thế giới!",
		],
		imageUrl: [
			"/Images/tours/bali/1.jpg",
			"/Images/tours/bali/4.jpg",
			"/Images/tours/bali/2.jpg",
			"/Images/tours/bali/3.jpg",
			"/Images/tours/bali/5.jpg",
			"/Images/tours/bali/6.jpg",
		],
		link: "https://www.netcodedev.com",
		price: "",
		rating: 4,
	},
	// {
	// 	id: "th3",
	// 	title: "Thailand",
	// 	description: [
	// 		"Thailand is a fascinating place that offers a wonderful combination of breathtaking scenery, deep-rooted culture, historical marvels, and bustling city life. Here's why Thailand deserves a top spot on your travel bucket list:",
	// 		"Thailand is renowned for its enchanting beaches, diverse cultural heritage, lively urban environment, delectable cuisine, thrilling escapades, wellness and relaxation opportunities, vibrant festivals, and the friendly nature of its people. The southern coastline boasts stunning beaches such as Phuket and the Phi Phi Islands, while Bangkok presents a fusion of contemporary skyscrapers and ancient temples. The cuisine encompasses everything from tantalizing street food to upscale dining, featuring specialties like pad Thai and green curry. Adventurous souls can explore mountainous regions, engage in world-class diving, or unwind at wellness resorts. Festivals like Songkran and Loy Krathong provide unique cultural experiences. Throughout the country, welcoming locals fosters a hospitable atmosphere for visitors.",
	// 		"Thailand is a destination that delivers a perfect blend of natural splendour, deep-rooted culture, bustling city life, and exhilarating adventures. Whether you seek to unwind on stunning beaches, explore ancient temples, savour delectable cuisine, or partake in thrilling activities, Thailand offers something for every traveller. So, prepare your bags and get ready to soak in the enchantment of Thailand to understand why it's one of the world's most adored travel destinations!",
	// 	],
	// 	imageUrl: [
	// 		"/Images/tours/vietnam/6.jpg",
	// 		"/Images/tours/vietnam/1.jpg",
	// 		"/Images/tours/vietnam/4.jpg",
	// 		"/Images/tours/vietnam/2.jpg",
	// 		"/Images/tours/vietnam/3.jpg",
	// 		"/Images/tours/vietnam/5.jpg",
	// 		"/Images/tours/vietnam/7.jpg",
	// 		"/Images/tours/vietnam/8.jpg",
	// 		"/Images/tours/vietnam/9.jpg",
	// 		"/Images/tours/vietnam/10.jpg",
	// 	],
	// 	link: "https://www.netcodedev.com",
	// 	price: "Adult: $120, Children < 10 years old:  $90",
	// 	rating: 4,
	// },
	{
		id: "ja4",
		title: "Japan - (04/2025 - 11/2025)",
		subTitle: [
			{
				id: "ja4.1",
				title: "LỊCH TRÌNH TOUR NHẬT BẢN",
				price:
					"AUD$4,800/khách. Trẻ em dưới 8 tuổi: AUD$3,700, trẻ dưới 2 tuổi: AUD$2,100/bé. (đặt sau 15/1/2025 giá tour sẽ cộng thêm 20%)",
				departs: "Tháng 11/2024 và 04/2025",
				schedule: [
					"Tour đã bao gồm: ",
					"* Vé máy bay khứ hồi Sydney- Nhật Bản,",
					"* Ăn sáng và ăn tối trong đoàn, (ăn trưa tự túc tại các khu phố ẩm thực có hướng dẫn của hướng dẫn viên). Cả đoàn sẽ được tổ chức 1 bữa ăn dạng tiệc tại nhà hàng nếu đoàn trên 15 khách lớn,",
					"* Nước uống suốt hành trình,",
					"* Hướng dẫn viên đi cùng từ Úc và suốt tuyến. Hướng dẫn viên địa phương sống tại Nhật*,",
					"* Vé vào tất cả các điểm tham quan trong chương trình,",
					"* Khách sạn tiêu chuẩn 3-5 sao (2 người 1 phòng hoặc phòng chung cho gia đình). Ở phòng riêng 1 người phụ thu AUD$400/khách,",
					"* Trải nghiệm hết những đặc sắc nhất 5 tỉnh nổi tiếng tại Nhật,",
					"* Vé tàu siêu tốc bullet Shinkansen,",
					"* Xe đưa đón tại Nhật và đưa đón sân bay tại Nhât Bản,",
					"",
					"* ***Tip cho Hướng dẫn viên: AUD$10/ngày/khách cho đoàn trên 10 khách., hoặc AUD$15/ngày/khách nếu đoàn dưới 10 khách. ",
					"* Quà lưu niệm từ đặc sản địa phương của Nhật tặng khách. ",
					"",
					"NGÀY 1 (20/03):  BAY TỪ SYDNEY ĐẾN TOKYO",
					"Xe đón tại sân bay về khách sạn 8-10pm. ",
					"",
					"NGÀY 2 (21/03): NARITA - TOKYO",
					"* Đoàn nhập cảnh, xe đón đoàn tại khách sạn. ",
					"* Tham quan bờ sông Meguro nơi ngăm hoa Anh đảo đẹp nhât Tokyo ",
					"* Ăn trưa",
					"* Tham quan hoàng cung nơi nhà vua sống. ",
					"* Tham quan vịnh Odaiba và check in tượng nữ thần tự do. ",
					"* Check in khách sạn nghỉ ngơi ăn tối.",
					"",
					"NGÀY 3 (22/03): PHÚ SỸ - FUJI ",
					"* Đoàn khởi hành đi Phú sĩ. ",
					"* Tham quan làng cổ Oshino Hakkai",
					"* Ăn trưa",
					"* Tham quan tháp 5 tầng Chureito",
					"* Tham quan và đi dạo ngăm hoa anh đào tại hồ Kawaguchi",
					"* Check in khách sạn nghỉ ngơi ăn tối.",
					"",
					"NGÀY 4 (23/03): HAKONE",
					"* Đoàn khởi hành di Hakone",
					"* Tham quan thung lũng núi lửa đang bốc khói Owakudani, ăn trứng đen thọ 7 năm tuổi",
					"* Tham quan và trải nghiệm tàu Hải tặc trên hồ Ashi",
					"* Ăn trưa",
					"* Khời hành đi Osaka",
					"* Check in khách sạn nghỉ ngơi ăn tối.",
					"",
					"NGÀY 5 (24/03): KYOTO",
					"* Đoàn khời hành đi Kyoto",
					"* Tham quan chùa Vàng (biểu tượng Koyto)",
					"* Ăn trưa",
					"* Tham quan và trãi nghiệm các con phố ở Kyoto",
					"*   Phố Gion..",
					"*   Chợ hải sản Nishiki",
					"*   Tham quan rừng trúc và khu du lịch Arashiyama",
					"* Check in khach san nghi ngơi. ",
					"* Ăn tối.",
					"",
					"NGÀY 6 (25/03): NARA - OSAKA",
					"* Đoàn khởi hành đi Nara",
					"* Tham quan công viên hưu sao cúi chào người và chùa Todaji",
					"* Ăn trưa",
					"* Đoàn di chuyền về trung tâm Osaka mua sắm và khám phá các con phố nồi tiếng : Dotonbori, Namba, Shinsaibashi.",
					"* Check in khách sạn nghỉ ngơi ăn tôi",
					"",
					"* NGÀY 7 (26/03): OSAKA - TOKYO",
					"* Đoàn check out khách sạn",
					"* Tham quan thành cổ Osaka ",
					"* Ăn trưa ",
					"* Đoàn di chuyển về Tokyo bằng Shikansen (SHINOSAKA-TOKYO)",
					"* Ăn tối ",
					"* Xe đón đoàn tại ga Tokyo, tiễn doàn ra sân bay Narita.",
					"NGÀY 7 ( 27/03): TRẢ KHÁCH TẠI SÂN BAY SYDNEY.",
					"",
					"*** Khách vui lòng đọc kỹ chương trình và đăng ký chuyển khoản giữ chỗ ngay AUD$2,500/người x tổng số người đi. ",
					"Hoặc đến văn phòng đại diện tại Cabramatta - Địa chỉ: 11 Burradoo Road, Lansvale, NSW 2166",
					"Liên hệ Ann qua số điện thoại 0401 211 890. Liên hệ trước khi tới 1 ngày để Ann sắp xếp không đi dẫn đoàn. ",
				],
			},
		],
		description: [
			"Nhật Bản là sự kết hợp quyến rũ giữa truyền thống cổ xưa và sự đổi mới hiện đại. Nhật Bản mang đến nhiều trải nghiệm đa dạng, từ những ngôi đền yên bình và những khu vườn xanh tươi đến những thành phố sôi động và công nghệ tiên tiến. Đây là lý do tại sao Nhật Bản nên là điểm đến du lịch tiếp theo của bạn:",
			"Di sản văn hóa phong phú của Nhật Bản trải dài hàng thiên niên kỷ, với các địa danh lịch sử như Kinkaku-ji (Golden Pavilion) ở Kyoto và Chùa Todai-ji ở Nara, nơi có tượng Phật bằng đồng lớn nhất thế giới. Tokyo kết hợp hoàn hảo giữa lịch sử và hiện đại ở những nơi như quận Asakusa và Đền Senso-ji có rất nhiều vẻ đẹp tự nhiên, từ sự hiện diện hùng vĩ của Núi Phú Sĩ đến mùa hoa anh đào (sakura) và tán lá mùa thu rực rỡ (koyo) ở Nikko và dãy Alps Nhật Bản.",
			"Những cuộc phiêu lưu trong đô thị đang chờ đón bạn với sự kết hợp của những ngôi đền truyền thống và những con phố được thắp sáng bằng đèn neon cũng như khung cảnh ẩm thực sôi động của Osaka ở quận Dotonborn. Thưởng thức các món ăn nổi tiếng thế giới như sushi, ramen và kaiseki ở Kyoto. Trải nghiệm văn hóa độc đáo bao gồm các nghi lễ trà truyền thống , các trận đấu vật sumo, biểu diễn kịch kabuki và các điểm tham quan tương lai như bảo tàng teamLab Borderless ở Tokyo.",
			"Lễ hội Nhật Bản tôn vinh truyền thống với các sự kiện như Gion Matsuri vào tháng 7 và Lễ hội tuyết Sapporo vào mùa đông. Lòng hiếu khách của đất nước bắt nguồn từ omotenashi, đảm bảo rằng du khách cảm thấy được chào đón ở nhiều nơi lưu trú khác nhau.",
			"Nhật Bản là điểm đến mang đến sự kết hợp đặc biệt giữa truyền thống cổ xưa, cảnh quan thiên nhiên tuyệt đẹp, công nghệ tiên tiến và trải nghiệm văn hóa độc đáo. Cho dù bạn đang khám phá những ngôi đền lịch sử, thưởng thức ẩm thực tinh tế, tham gia các cuộc phiêu lưu trong đô thị hay tham gia các lễ hội truyền thống , Nhật Bản có điều gì đó làm say mê mọi du khách. Vì vậy, hãy chuẩn bị hành lý và bắt đầu chuyến hành trình khó quên đến Xứ sở mặt trời mọc!",
		],
		imageUrl: [
			"/Images/tours/japan/1.jpg",
			"/Images/tours/japan/2.jpg",
			"/Images/tours/japan/3.jpg",
			"/Images/tours/japan/4.jpg",
			"/Images/tours/japan/5.jpg",
			"/Images/tours/japan/6.jpg",
			"/Images/tours/japan/7.jpg",
			"/Images/tours/japan/8.jpg",
			"/Images/tours/japan/9.jpg",
			"/Images/tours/japan/10.jpg",
			"/Images/tours/japan/11.jpg",
			"/Images/tours/japan/12.jpg",
			"/Images/tours/japan/13.jpg",
			"/Images/tours/japan/14.jpg",
			"/Images/tours/japan/15.jpg",
			"/Images/tours/japan/16.jpg",
			"/Images/tours/japan/17.jpg",
			"/Images/tours/japan/18.jpg",
			"/Images/tours/japan/19.jpg",
			"/Images/tours/japan/20.jpg",
			"/Images/tours/japan/21.jpg",
			"/Images/tours/japan/22.jpg",
		],
		link: "https://www.netcodedev.com",
		price: "",
		rating: 4,
	},
	// {
	// 	id: "ko5",
	// 	title: "South Korea",
	// 	description: [
	// 		"South Korea is an energetic and diverse country that provides a distinctive mix of age-old customs, contemporary innovations, stunning natural beauty, and thrilling urban adventures. Here's why you should consider South Korea for your next travel destination:",
	// 		"South Korea boasts a rich cultural legacy, including UNESCO World Heritage sites such as Changdeokgung Palace and Gyeongbokgung Palace. Take in the traditional lifestyle at Bukchon Hanok Village with its well-preserved hanok houses. In Seoul, ancient palaces and temples harmoniously coexist with modern skyscrapers and futuristic structures in areas like Gangnam and Hongdae.",
	// 		"Busan offers gorgeous beaches, vibrant seafood markets, and cultural festivities at Haeundae Beach and Gwangalli Beach. The lively atmosphere of Jagalchi Fish Market, where fresh seafood is abundant, is a must-visit. South Korea's natural beauty encompasses Hallasan Mountain on Jeju Island and the rugged mountains and forests of Seoraksan National Park.",
	// 		"Korean cuisine boasts bold flavors, diverse dishes, and a focus on fresh ingredients, such as bibimbap, bulgogi, and kimchi. In Seoul, you can savor street foods like tteokbokki, hotteok, and kimbap. Busan is renowned for its seafood, particularly sashimi and haemul jjigae.",
	// 		"One-of-a-kind cultural experiences in South Korea include tea ceremonies, staying in hanoks, visiting taekwondo gyms, enjoying pansori performances and samulnori music, and partaking in vibrant festivals like the Lantern Festival, Boryeong Mud Festival, Jinhae Cherry Blossom Festival, and Andong Mask Dance Festival.",
	// 		"South Korea's vibrant pop culture encompasses K-pop, K-dramas, fashion, and trendy neighborhoods like Myeongdong and Dongdaemun. The transportation system is efficient, with high-speed trains and subways in major cities. South Koreans are known for their warm hospitality and the concept of jeong.",
	// 		"South Korea is a destination that offers an ideal combination of age-old traditions, modern innovations, breathtaking natural landscapes, and vibrant urban experiences. Whether you're exploring historic palaces, relishing delicious cuisine, partaking in cultural festivals, or immersing yourself in pop culture, South Korea has something for every traveller. So, get your bags packed, and get ready to delve into the marvels of South Korea – truly one of the world's most enthralling travel destinations!",
	// 	],
	// 	imageUrl: [
	// 		"/Images/tours/vietnam/8.jpg",
	// 		"/Images/tours/vietnam/4.jpg",
	// 		"/Images/tours/vietnam/5.jpg",
	// 		"/Images/tours/vietnam/6.jpg",
	// 		"/Images/tours/vietnam/1.jpg",
	// 		"/Images/tours/vietnam/2.jpg",
	// 		"/Images/tours/vietnam/3.jpg",
	// 		"/Images/tours/vietnam/7.jpg",
	// 		"/Images/tours/vietnam/9.jpg",
	// 		"/Images/tours/vietnam/10.jpg",
	// 	],
	// 	link: "https://www.netcodedev.com",
	// 	price: "Adult: $120, Children < 10 years old:  $90",
	// 	rating: 4,
	// },
	// {
	// 	id: "us6",
	// 	title: "The United States",
	// 	description: [
	// 		"The USA is an extensive and diverse country that offers a variety of experiences, from awe-inspiring natural landscapes and lively cities to a rich cultural heritage and innovative attractions. Here's why a trip to America should be next on your travel agenda:",
	// 		"The United States boasts stunning natural wonders such as the Grand Canyon and Yellowstone National Park. Its vibrant cities like New York City and Los Angeles are renowned for their rich cultural scenes and entertainment options. The country holds a significant cultural legacy, with iconic landmarks in Washington, D.C., and musical hotspots in New Orleans. Additionally, it provides thrilling outdoor activities in places like the Rocky Mountains and the Florida Keys. The diverse American cuisine and unique cultural experiences, from Broadway shows in New York to live sports events, further contribute to the country's appeal. Moreover, the United States hosts a wide range of festivals and events, and visitors can anticipate encountering welcoming locals and experiencing warm hospitality throughout the country.",
	// 		"The United States is a destination that offers an extraordinary blend of natural beauty, lively cities, significant cultural heritage, and thrilling adventures. Whether you're exploring iconic landmarks, savoring delightful cuisine, enjoying cultural festivals, or embarking on outdoor adventures, the United States has something to captivate every traveller. So, pack your bags and get ready to uncover the wonders of America and discover why it's one of the world's most diverse and dynamic travel destinations!",
	// 	],
	// 	imageUrl: [
	// 		"/Images/tours/vietnam/7.jpg",
	// 		"/Images/tours/vietnam/10.jpg",
	// 		"/Images/tours/vietnam/8.jpg",
	// 		"/Images/tours/vietnam/4.jpg",
	// 		"/Images/tours/vietnam/5.jpg",
	// 		"/Images/tours/vietnam/6.jpg",
	// 		"/Images/tours/vietnam/1.jpg",
	// 		"/Images/tours/vietnam/2.jpg",
	// 		"/Images/tours/vietnam/3.jpg",
	// 		"/Images/tours/vietnam/9.jpg",
	// 	],
	// 	link: "https://www.netcodedev.com",
	// 	price: "Adult: $120, Children < 10 years old:  $90",
	// 	rating: 4,
	// },
	// {
	// 	id: "ur7",
	// 	title: "Europe",
	// 	description: [
	// 		"Europe boasts a rich history, diverse culture, and stunning natural beauty, making it an ideal destination for all types of travellers. From ancient landmarks and charming villages to vibrant cities and breathtaking landscapes, Europe has it all. Here’s why you should consider Europe for your next travel adventure:",
	// 		"Europe is home to iconic historical landmarks such as the Eiffel Tower, Notre Dame Cathedral, and the Colosseum in Italy. The continent's vibrant cities and culture are exemplified in London, Barcelona, and Amsterdam, while its stunning natural landscapes include the Swiss Alps, Norwegian fjords, and the Amalfi Coast.",
	// 		"European cuisine encompasses diverse dishes such as coq au vin in France, pasta and pizza in Italy, tapas and paella in Spain, and traditional English breakfast in the UK. Unique cultural experiences include Oktoberfest in Germany, the Carnival of Venice, and Christmas markets in Austria and Germany.",
	// 		"Europe also has a rich artistic and musical heritage, with notable galleries and museums in Madrid, Florence, and Paris. The continent hosts a wide range of festivals and events, including the Edinburgh Festival Fringe, La Tomatina in Spain, the Running of the Bulls in Pamplona, and the Cannes Film Festival in France.",
	// 		"Europe offers an extraordinary blend of historical landmarks, vibrant cities, stunning natural landscapes, delicious cuisine, unique cultural experiences, and rich artistic heritage. Whether you’re exploring ancient ruins, indulging in gourmet meals, enjoying cultural festivals, or embarking on outdoor adventures, Europe has something to captivate every traveller. So, pack your bags and get ready to discover the wonders of Europe and why it’s one of the world’s most enchanting travel destinations!",
	// 	],
	// 	imageUrl: [
	// 		"/Images/tours/vietnam/2.jpg",
	// 		"/Images/tours/vietnam/6.jpg",
	// 		"/Images/tours/vietnam/7.jpg",
	// 		"/Images/tours/vietnam/10.jpg",
	// 		"/Images/tours/vietnam/8.jpg",
	// 		"/Images/tours/vietnam/4.jpg",
	// 		"/Images/tours/vietnam/5.jpg",
	// 		"/Images/tours/vietnam/1.jpg",
	// 		"/Images/tours/vietnam/3.jpg",
	// 		"/Images/tours/vietnam/9.jpg",
	// 	],
	// 	link: "https://www.netcodedev.com",
	// 	price: "Adult: $120, Children < 10 years old:  $90",
	// 	rating: 4,
	// },
	// {
	// 	id: "nz8",
	// 	title: "New Zealand",
	// 	description: [
	// 		"New Zealand offers an unparalleled blend of natural beauty, rich Maori culture, and exciting adventures. This South Pacific gem provides diverse experiences, from tranquil landscapes and vibrant cities to unique wildlife and outdoor activities.",
	// 		"New Zealand boasts stunning landscapes, from snow-capped mountains to lush rainforests. Must-visit spots include Milford Sound in Fiordland National Park, Aoraki/Mount Cook for hiking, and Rotorua's geothermal marvels like Wai-O-Tapu Thermal Wonderland.",
	// 		"New Zealand boasts stunning landscapes, from snow-capped mountains to lush rainforests. Must-visit spots include Milford Sound in Fiordland National Park, Aoraki/Mount Cook for hiking, and Rotorua's geothermal marvels like Wai-O-Tapu Thermal Wonderland.",
	// 		"Maori culture is deeply rooted in New Zealand, with Te Puia showcasing traditional arts and Waitangi Treaty Grounds offering historical insights. Immersive cultural experiences can be had through Marae’s visits.",
	// 		"Adventure enthusiasts can skydive over Queenstown, bungee jump at Kawarau Bridge, or trek the breathtaking Tongariro Alpine Crossing. Wildlife lovers can spot unique species such as kiwi birds and marine life in Kaikoura.",
	// 		"Charming cities like Auckland, Wellington, and Queenstown effortlessly blend modern amenities with local culture. New Zealand's cuisine features fresh seafood and farm-to-table dining, paired with world-class wines from regions like Marlborough and Central Otago.",
	// 		"Kiwis are famous for their warm hospitality, ensuring a welcoming experience in various accommodations across the country. In essence, New Zealand offers an extraordinary mix of natural beauty, rich culture, thrilling adventures, unique wildlife, and captivating cities. Whether you're exploring breathtaking landscapes, immersing yourself in Maori culture, enjoying outdoor activities, or savouring delicious cuisine and wine, New Zealand has something to captivate every traveller. So, pack your bags and get ready to discover the wonders of New Zealand, one of the world's most enchanting travel destinations!",
	// 	],
	// 	imageUrl: [
	// 		"/Images/tours/vietnam/2.jpg",
	// 		"/Images/tours/vietnam/6.jpg",
	// 		"/Images/tours/vietnam/7.jpg",
	// 		"/Images/tours/vietnam/10.jpg",
	// 		"/Images/tours/vietnam/8.jpg",
	// 		"/Images/tours/vietnam/4.jpg",
	// 		"/Images/tours/vietnam/5.jpg",
	// 		"/Images/tours/vietnam/1.jpg",
	// 		"/Images/tours/vietnam/3.jpg",
	// 		"/Images/tours/vietnam/9.jpg",
	// 	],
	// 	link: "https://www.netcodedev.com",
	// 	price: "Adult: $120, Children < 10 years old:  $90",
	// 	rating: 4,
	// },
];
