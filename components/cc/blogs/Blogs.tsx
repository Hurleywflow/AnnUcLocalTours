...
const Blogs = () => {
	...
	const blogDataViSlice = blogDataVi.slice(0, blogDataVi.length - 1);
						...
						<div className='grid h-fit grid-cols-2 gap-1 space-y-5 md:grid-cols-3 md:gap-2 lg:grid-cols-4'>
							...
							<div className='col-span-2 flex cursor-pointer flex-col gap-4  md:col-span-3 lg:col-span-4'>
								{pathname.includes("/vi") ? (
									<Link
										href={`/${locale}/blogs/${blogDataVi[blogDataVi.length - 1].id}`}
										key={blogDataVi[blogDataVi.length - 1].id}
									>
										<div className='relative aspect-video overflow-hidden  rounded-md bg-muted'>
											<Image
												src={
													blogDataVi[blogDataVi.length - 1].imageUrl[
														blogDataVi.length - 1
													]
												}
												alt={blogDataVi[blogDataVi.length - 1].title}
												fill
										...
										<div className='mt-2 flex flex-row items-center gap-4'>
											...
											<p className='flex flex-row items-center gap-2 text-sm'>
												...
												<span>{blogDataVi[blogDataVi.length - 1].author}</span>
											</p>
										</div>
										<div className='flex flex-col gap-2'>
											<H5 className='text-start'>
												{blogDataVi[blogDataVi.length - 1].title}
											</H5>
											<p className=' truncate text-base text-muted-foreground'>
												{blogDataVi[blogDataVi.length - 1].description}
											</p>
										</div>
									</Link>
								) : (
